...
class DashboardController extends Controller
{
    ...
    public function quotationData(Request $request): JsonResponse
    {
        ...
        $data = $quotations->map(function ($q) use ($user, $canViewAll, $canDownload, $canDownloadBranded, $canDownloadPlain, $canDelete, $canConvert, $canHold, $canCancel, $canResume) {
            ...
            return [
                ...
                'referral_name' => $q->referral_name,
                'referral_type_label' => $q->referral_type_label,
                ...
                'convert_url' => ($canConvert && ! $q->loan_id)
                    ...
                    : null,
                'edit_url' => $q->isEditableBy($user)
                    ? route('quotations.edit', $q->id)
                    : null,
                'loan_url' => $q->loan_id
    ...
    public function dvrData(Request $request): JsonResponse
    {
        ...
        $data = $visits->map(function (DailyVisitReport $visit) use ($contactTypeLabels, $purposeLabels, $countFollowUps) {
    ...
    private function newthemeOpenQueries(User $user): array
    {
        ...
        // queries assigned to them OR on a loan visible to them (owner / advisor /
        // stage assignment / branch / transfer history — same rule as the loan list).
        if (! $user->hasPermission('view_all_loans')) {
            $query->where(function ($q) use ($user) {
                $q->where('assigned_to_user_id', $user->id)
                    ->orWhereHas('loan', fn ($lq) => $lq->visibleTo($user));
            });
        }
