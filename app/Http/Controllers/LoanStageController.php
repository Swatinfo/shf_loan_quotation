...
class LoanStageController extends Controller
{
    ...
    public function index(LoanDetail $loan)
    {
        ...
        $skipAllowed = [];
        if ($loan->product_id) {
            $productStages = ProductStage::where('product_id', $loan->product_id)
                ->with('stage')
                ->get();
    ...
    public function resolveQuery(StageQuery $query): JsonResponse
    {
        if ($query->status === 'resolved') {
            return response()->json(['error' => 'This query is already resolved.'], 422);
        }

        // Resolvable by the raiser, the current assignee of the query's stage,
        // or admin/super_admin — regardless of pending/responded status.
        $user = auth()->user();
        $canResolve = $query->raised_by === $user->id
            || ($query->stageAssignment && $query->stageAssignment->assigned_to === $user->id)
            || $user->hasAnyRole(['super_admin', 'admin']);

        if (! $canResolve) {
            return response()->json(['error' => 'Only the query raiser, the current stage assignee, or an admin can resolve this query.'], 403);
        }

        $this->queryService->resolveQuery($query, $user->id);
    ...
    {
        $validated = $request->validate([
            'action' => 'required|in:send_to_bank,initiate_legal,complete_skip_bank',
        ...
        if ($validated['action'] === 'complete_skip_bank') {
            if (! $this->canSkipLegalBank($loan)) {
            ...
            return response()->json(['success' => true, 'message' => 'Legal verification completed (bank-side skipped). Original Document Verification auto-completed.']);
        }
    ...
    public function saveNotes(Request $request, LoanDetail $loan, string $stageKey): JsonResponse
    {
        ...
        // DME is locked once the Application Number sub-stage is complete — only
        // the dedicated change-DME action (super_admin/admin/bdh) may alter it
        // afterwards. Strip any attempt to change it through the stage form.
        if ($stageKey === 'app_number' && $assignment->status === 'completed') {
            unset($notesData['dme_user_id']);
        }
        ...
        // Mirror the DME to the loan column (single source of truth, queryable +
        // used by the change-DME action). Only while the sub-stage is open.
        if ($stageKey === 'app_number' && $assignment->status !== 'completed' && ! empty($notesData['dme_user_id'])) {
            $loan->update(['dme_user_id' => (int) $notesData['dme_user_id']]);
        }

        // Mirror the sanctioned amount to its dedicated loan column (queryable, used
        // by listings). Captured at docket login; legacy loans use the sanction stage.
        // Docket always wins — only let the sanction stage fill an empty column.
        if (isset($notesData['sanctioned_amount']) && is_numeric($notesData['sanctioned_amount'])) {
            if ($stageKey === 'docket') {
                $loan->update(['sanctioned_amount' => (int) $notesData['sanctioned_amount']]);
            } elseif ($stageKey === 'sanction' && $loan->sanctioned_amount === null) {
                $loan->update(['sanctioned_amount' => (int) $notesData['sanctioned_amount']]);
            }
        }
        ...
        if ($stageKey === 'sanction' && ! empty($notesData['sanction_date'])) {
            ...
            if ($docketOffset && $docketOffset !== '0') {
                $sanctionDateObj = Carbon::createFromFormat('d/m/Y', $notesData['sanction_date']);
                ...
                $loan->update(['due_date' => Carbon::createFromFormat('d/m/Y', $appNotes['custom_docket_date'])->toDateString()]);
            }
        }
    ...
    private function getFieldErrors(string $stageKey, array $data): array
    {
        ...
        $rules = match ($stageKey) {
            'app_number' => array_merge(
                ['application_number' => 'Application Number', 'dme_user_id' => 'DME', 'docket_days_offset' => 'Docket Timeline'],
                ($data['docket_days_offset'] ?? '') === '0' ? ['custom_docket_date' => 'Custom Docket Date'] : []
            ),
            'bsm_osv' => [],
            'legal_verification' => [],
            'original_document_verification' => ['verification_date' => 'Verification Date'],
     ...
     */
    /**
     * Authority gate for the legal-verification "complete without sending to
     * bank" action. Allowed for: super_admin, the loan creator, the loan's
     * assigned advisor, or any branch_manager / bdh attached to the loan's
     * branch. Bank employees are explicitly NOT allowed here — they would
     * never see Phase 1 anyway, but the server-side check is the source of
     * truth.
     */
    private function canSkipLegalBank(LoanDetail $loan): bool
    {
        $user = auth()->user();
        if (! $user) {
            return false;
    ...
    private function isStageDataComplete(string $stageKey, $assignment): bool
    {
        ...
        return match ($stageKey) {
            ...
            'bsm_osv' => true,
            'legal_verification' => in_array($notesData['legal_phase'] ?? '', ['3', 'completed_skip_bank'], true),
            'original_document_verification' => ! empty($notesData['verification_date']),
    ...
    /**
     * Backward-compatible entry point for sanction_decision.
     */
    public function sanctionDecisionAction(Request $request, LoanDetail $loan): JsonResponse
    {
        return $this->decisionAction($request, $loan, 'sanction_decision');
    }

    /**
     * Escalation-ladder decision handler shared by sanction_decision and
     * technical_valuation. The escalation LEVEL is derived from the stage's
     * `escalation_history` (last to_role) — independent of the dynamic base
     * role assigned via product-stage config:
     *   - base : not yet escalated → current holder may Escalate to BM
     *   - bm   : escalated to branch_manager → BM may Escalate to BDH
     *   - bdh  : escalated to bdh → terminal
     * super_admin / admin bypass every level/role guard.
     *
     * sanction_decision supports `approve` (sanction the loan); technical_valuation
     * does NOT — its valuation form save is the approval (handled elsewhere).
     */
    public function decisionAction(Request $request, LoanDetail $loan, string $stageKey): JsonResponse
    {
        abort_unless(in_array($stageKey, ['sanction_decision', 'technical_valuation'], true), 404);

        $allowedActions = $stageKey === 'sanction_decision'
            ? 'approve,escalate_to_bm,escalate_to_bdh,reject'
            : 'escalate_to_bm,escalate_to_bdh,reject';

        $validated = $request->validate([
            'action' => 'required|in:'.$allowedActions,
        ...
        // Escalation level from history (last to_role), independent of base role.
        $history = $notesData['escalation_history'] ?? [];
        $lastRole = $history ? ($history[count($history) - 1]['to_role'] ?? null) : null;
        $level = match ($lastRole) {
            'branch_manager' => 'bm',
            'bdh' => 'bdh',
            default => 'base',
        };

        // ── Approve (sanction_decision only) ──
        if ($action === 'approve') {
            // Guard BEFORE mutating: updateStageStatus() throws on unresolved
            // queries / invalid transitions, which previously left the loan
            // half-approved (is_sanctioned set, stage still in_progress) and
            // surfaced as a blank 500 in the UI.
            if ($assignment->hasPendingQueries()) {
                $pendingCount = $assignment->queries()->whereIn('status', ['pending', 'responded'])->count();

                return response()->json(['error' => "Cannot approve — {$pendingCount} unresolved query/queries on this stage. Resolve all queries first."], 422);
            }

            if ($assignment->status !== 'completed' && ! $assignment->canTransitionTo('completed')) {
                return response()->json(['error' => "Cannot approve — stage is '{$assignment->status}' and cannot be completed from that status."], 422);
            }

            $loan->update(['is_sanctioned' => true]);
            $assignment->mergeNotesData(['decision_action' => 'approved', 'decided_by' => $user->id]);
            $this->stageService->updateStageStatus($loan, $stageKey, 'completed', $user->id);
            $loan->refresh();
            $progress = $this->stageService->recalculateProgress($loan);
            ...
            return response()->json([
                ...
                'assignment' => ['stage_key' => $stageKey, 'status' => 'completed'],
                'current_stage' => $loan->current_stage,
                'progress' => ['completed' => $progress->completed_stages, 'total' => $progress->total_stages, 'percentage' => $progress->overall_percentage],
            ]);
        }

        // ── Escalate (requires remarks) ──
        if (in_array($action, ['escalate_to_bm', 'escalate_to_bdh'])) {
            ...
            if (! $isAdmin) {
                if ($action === 'escalate_to_bm' && $level !== 'base') {
                    return response()->json(['error' => 'This task has already been escalated.'], 422);
                }
                if ($action === 'escalate_to_bdh' && ! ($level === 'bm' && $user->hasRole('branch_manager'))) {
                    return response()->json(['error' => 'Only a Branch Manager can escalate to BDH.'], 403);
                }
            }
            ...
            if ($transferTo) {
                $this->stageService->transferStage($loan, $stageKey, (int) $transferTo, "Escalated to {$targetRole}: {$validated['decision_remarks']}");
            }
        ...
        // ── Reject (requires reason; bm/bdh level + BM/BDH, or admin) ──
        if ($action === 'reject') {
            ...
            if (! $isAdmin && ! (in_array($level, ['bm', 'bdh'], true) && $user->hasAnyRole(['branch_manager', 'bdh']))) {
            ...
            $this->stageService->rejectLoan($loan, $stageKey, $validated['rejection_reason'], $user->id);
