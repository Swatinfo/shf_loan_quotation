...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->normalizeAssignedBanks($request);

        $allRoleSlugs = Role::pluck('slug')->toArray();
    ...
    protected function getAllowedRoles(): array
    {
        ...
        $roles = Role::orderBy('id')->pluck('name', 'slug')->toArray();
    ...
    /**
     * Strip empty values from the submitted assigned_banks input.
     *
     * The role-conditional bank <select> stays in the DOM (hidden) for non-bank
     * roles and posts its empty placeholder option, which would otherwise fail
     * the `exists:banks,id` rule with "The selected assigned_banks.0 is invalid".
     */
    protected function normalizeAssignedBanks(Request $request): void
    {
        $request->merge([
            'assigned_banks' => array_values(array_filter(
                (array) $request->input('assigned_banks', []),
    ...
    protected function replaceProductStageUsers(User $user, Request $request, array $validated): void
    {
        ...
        $roleMatchingStageIds = Stage::where('is_enabled', true)->get()
     ...
     */
    public function productStageHolders(Request $request): JsonResponse
    {
