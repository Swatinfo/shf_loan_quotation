...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        $loanBanks = Bank::active()->orderBy('name')->pluck('name')->toArray();
    ...
    public function updateQuotationReferralTypes(Request $request)
    {
        $validated = $request->validate([
            'quotationReferralTypes' => 'required|array|min:1',
            'quotationReferralTypes.*.key' => 'required|string|max:500',
            'quotationReferralTypes.*.label_en' => 'required|string|max:10000',
            'quotationReferralTypes.*.label_gu' => 'required|string|max:10000',
        ]);

        $this->configService->updateSection('quotationReferralTypes', array_values($validated['quotationReferralTypes']));
        ActivityLog::log('settings_updated', null, ['section' => 'quotation_referral_types']);

        return back()->with('success', 'Quotation referral types updated.');
    }
