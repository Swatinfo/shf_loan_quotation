...
class LoanConversionService
{
    ...
    public function convertFromQuotation(Quotation $quotation, int $bankIndex, array $extra = []): LoanDetail
    {
        ...
        return DB::transaction(function () use ($quotation, $quotationBank, $extra) {
            ...
            // never updating an existing master). The per-loan KYC snapshot is
            // recorded below after the loan exists.
            $kyc = [
            ...
            $customer = $this->customerService->resolveMasterByPan($kyc);
            ...
            // Record the per-loan KYC snapshot and link the loan to it.
            $kycRow = $this->customerService->recordKyc($customer, $kyc, [
                'loan_id' => $loan->id,
                'quotation_id' => $quotation->id,
                'source' => 'conversion',
            ]);
            $loan->update(['customer_kyc_details_id' => $kycRow->id]);
    ...
    public function createDirectLoan(array $data): LoanDetail
    {
        return DB::transaction(function () use ($data) {
            ...
            // Resolve customer identity by PAN + record the per-loan KYC snapshot.
            $kyc = [
                'customer_name' => $data['customer_name'],
                'mobile' => $data['customer_phone'] ?? null,
            ...
            $loan->update([
                'customer_id' => $customer->id,
                'customer_kyc_details_id' => $kycRow->id,
            ]);
