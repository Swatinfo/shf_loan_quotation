...
|------|-------------|-------|
| `total` | Total quotations | All (admin) or own (staff) |
...
- Staff without that permission → see only own quotations + own stats
...
4. **My Pending Stages** (if user has task_role): List of stages assigned to current user that need action
