...
> **⚠️ Reverb removed.** Laravel Reverb / WebSocket broadcasting was removed from this project. `BROADCAST_CONNECTION=null`, `routes/channels.php` registers no channels, and the `NotificationBroadcast` event no longer exists. The in-app bell now relies on the **60s badge poll** (`GET /api/notifications/count`) — which also plays the in-app chime (`SHFPush.playChime`) when the unread count rises — plus **Web Push** for OS notifications when the app is closed. The Reverb sections below are kept **for historical reference only**; do not follow them unless you intend to re-introduce WebSocket broadcasting from scratch.
