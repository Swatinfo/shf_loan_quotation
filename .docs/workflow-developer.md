...
| `PermissionService` | `app/Services/PermissionService.php` | `userHasPermission`, `roleHasPermission`, `taskRoleHasPermission`, `getUserPermissions`, `clearAllCaches` |
| `ConfigService` | `app/Services/ConfigService.php` | `load`, `get`, `updateSection`, `updateMany`, `reset` |

### Models (17 loan-specific)

`LoanDetail`, `StageAssignment`, `LoanDocument`, `LoanProgress`, `StageTransfer`, `StageQuery`, `QueryResponse`, `ValuationDetail`, `DisbursementDetail`, `Remark`, `ShfNotification`, `Bank`, `Branch`, `Product`, `Stage`, `ProductStage`, `TaskRolePermission`
...
| 4a | `app_number` | Application Number | sequential | parallel_processing | bank_employee |
| 4b | `bsm_osv` | BSM/OSV Approval | sequential | parallel_processing | bank_employee |
| 4c | `legal_verification` | Legal Verification | sequential | parallel_processing | loan_advisor, branch_manager |
| 4d | `technical_valuation` | Technical Valuation | sequential | parallel_processing | loan_advisor, branch_manager |
| 4e | `property_valuation` | Property Valuation | sequential | parallel_processing | (product-specific, e.g. LAP) |
...
| 8 | `kfs` | KFS Generation | sequential | -- | bank_employee |
| 9 | `esign` | E-Sign & eNACH | sequential | -- | bank_employee |
| 10 | `disbursement` | Disbursement | decision | -- | loan_advisor, branch_manager |
| 11 | `otc_clearance` | OTC Clearance | sequential | -- | office_employee |
...
15 stage assignments created per loan. `property_valuation` is product-specific and added via product stage config.
...
**Phases:** `esign_phase` (1 -> 2 -> 3)

| Phase | Owner | Action |
|-------|-------|--------|
| 1 | Bank employee | Initial assignment |
| 2 | Task owner | Customer completion. Triggered by `esignAction(esign_generated)`. Bank employee ID tracked in `esign_bank_employee`. |
| 3 | Bank employee | Final confirmation. Triggered by `esignAction(esign_customer_done)`. Stage auto-completes. |
...
## Permission Resolution (4-tier)

### `PermissionService::userHasPermission(User, slug)`

| Priority | Check | Result |
|----------|-------|--------|
| 1 | `user.isSuperAdmin()` | Always `true` |
| 2 | `user_permissions` table (grant/deny) | Explicit override |
| 3 | `role_permissions` table (system role: admin/staff) | Role default |
| 4 | `task_role_permissions` table (task role: additive) | Task role grant |

### Caching

- User overrides: `user_perms:{userId}` (5 min TTL)
- Role permissions: `role_perms:{role}` (5 min TTL)
- Task role permissions: `task_role_perms:{taskRole}` (5 min TTL)
...
**Note:** `legal_advisor` exists in `User::TASK_ROLES` constant but is NOT in `ADVISOR_ELIGIBLE_ROLES`.

### Loan Creation Authorization

```php
User::canCreateLoans(): bool
// true if super_admin, admin, or task_role in ADVISOR_ELIGIBLE_ROLES
```

`ADVISOR_ELIGIBLE_ROLES = ['branch_manager', 'loan_advisor', 'office_employee']`
...
| Slug | Admin | Staff | Description |
|------|-------|-------|-------------|
| `convert_to_loan` | yes | yes | Convert quotation to loan |
| `view_loans` | yes | yes | View loan list |
| `view_all_loans` | yes | no | View all loans (not just own) |
| `create_loan` | yes | yes | Create new loan |
| `edit_loan` | yes | no | Edit loan details |
| `delete_loan` | yes | no | Delete loan |
| `manage_loan_documents` | yes | yes | Manage document collection |
| `manage_loan_stages` | yes | yes | Stage operations (status, assign, transfer, query) |
| `skip_loan_stages` | yes | no | Skip stages |
| `add_remarks` | yes | yes | Add remarks to loans |
| `manage_workflow_config` | yes | no | Manage loan settings (banks/products/branches/stages) |
...
```php
TASK_ROLES = ['branch_manager', 'loan_advisor', 'bank_employee', 'office_employee']
ADVISOR_ELIGIBLE_ROLES = ['branch_manager', 'loan_advisor', 'office_employee']
```
