...
**Docket-date override** — once `sanction` is complete and while the loan is **not `completed`**, holders of the `edit_docket_date` permission (default: super_admin/admin/branch_manager/bdh) can correct `loan_details.expected_docket_date` directly (completed loans hide the control and the endpoint returns 422). UI: a "Change/Set Docket Date" button in the header action row of both the loan **view** page and the loan **stages** page (the shared `newtheme.loans._docket-date-modal` partial — themed SweetAlert modal + Bootstrap datepicker). Endpoint `POST /loans/{loan}/docket-date` (`loans.docket-date.update`, `LoanController@updateDocketDate`) requires a **mandatory reason** and a date **≥ today** (`bail` + closure — `after_or_equal:today` mis-parses `d/m/Y`), records the old → new change + reason to `activity_log` (`change_docket_date`), and does **not** touch `due_date`. The app_number **custom docket date** (`docket_days_offset = 0`) is likewise constrained to ≥ today, enforced in `LoanStageController::getFieldErrors` and the datepicker `min_date`.

**Read-only stage detail for all viewers** — on the stages page, the main-stage financial detail (rate_pf / sanction / docket / disbursement figures) used to be wrapped in `@if ($stageEditable)` (= `$isMainAssignee`), so only the assignee/admin saw it. It's now split: `$stageViewable = status ∈ {in_progress, completed}` gates a read-only view, `$stageEditable = $stageViewable && $isMainAssignee` still gates every form/button. Non-assignee viewers (anyone who can open the loan) get the same figures via the `newtheme.loans._stage-readonly-detail` partial (`@elseif ($stageViewable)`), which contains **no** forms/inputs/buttons — important because stage-action endpoints are gated by the `manage_loan_stages` permission (not by assignee), so the blade's assignee-gating is the real guard against non-assignees acting. Covered by `StageReadonlyDetailTest`.
