...
self.addEventListener("fetch", function (event) {
    ...
    // but ALWAYS refetch in the background and update the cache, so a deploy
    // that forgot to bump SHF_VERSION/SHF_SW_VERSION self-heals one reload
    // later instead of pinning clients to pre-deploy CSS/JS forever
    // (2026-07-04: payout slabs "only worked after hard refresh").
...
function isStaticAsset(pathname) {
    if (pathname === OFFLINE_URL || pathname === "/manifest.json") {
        return true;
    }
    var exts = [
        ".css",
...
self.addEventListener("push", function (event) {
    ...
    try {
        ...
        data = { title: "SHF", body: event.data ? event.data.text() : "" };
    }
    ...
    event.waitUntil(
        self.registration.showNotification(title, options).then(function () {
            // If any app tab is open (foreground or background), ask it to play
            // the user's chosen chime immediately — the SW itself can't play
            // audio, and the chosen preset lives in the page's localStorage.
            // When no tab is open, only the OS default notification sound plays
            // (a closed PWA cannot play a custom sound — platform limitation).
            return self.clients
                .matchAll({ type: "window", includeUncontrolled: true })
                .then(function (clientList) {
                    clientList.forEach(function (c) {
                        c.postMessage({ type: "shf-play-chime" });
                    });
                });
        }),
    );
        ...
        self.clients
            ...
            .then(function (clientList) {
                for (var i = 0; i < clientList.length; i++) {
                    var c = clientList[i];
                    if (
                        c.url.indexOf(self.location.origin) === 0 &&
                        "focus" in c
                    ) {
                        if (c.navigate) {
                            c.navigate(target);
