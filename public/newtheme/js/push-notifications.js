...
(function () {
    ...
    // ============================================================
    //  In-tab chime, played by the bell-badge poll when the unread count rises.
    ...
    function setChimePreset(key) {
        ...
        // Let the native-shell bridge re-register the device token so the server
        // sends FCM on the channel matching the newly-chosen sound.
        try {
            window.dispatchEvent(new CustomEvent('shf-chime-preset-changed', { detail: { key: key } }));
        } catch (e) {}
        return true;
    }
    ...
    function playChime() {
        if (isMuted()) { return; }
        // De-dupe: the badge poll and the service-worker push message can both
        // ask to chime for the same notification within a second of each other.
        var now = Date.now();
        if (now - lastChimeAt < 2000) { return; }
        lastChimeAt = now;
