...
$(function () {
    ...
    SHF.validateForm = function ($form, rules) {
        ...
        $.each(rules, function (fieldName, rule) {
            ...
            if ($field.is(":radio")) {
                val =
                    $form.find('[name="' + fieldName + '"]:checked').val() ||
                    "";
            } else if (
                $field.is(":checkbox") &&
            ...
            var label =
                rule.label ||
                fieldName.replace(/_/g, " ").replace(/\b\w/g, function (c) {
                    ...
                    if (isNaN(numVal)) {
                        ...
                        if (
                            ...
                            err =
                                label +
                                " must not exceed " +
                                SHF.formatIndianNumber(rule.max) +
                                ".";
                        }
                    }
                }
        ...
        if (errors.length) {
            ...
            var errStyle =
                "display:block;width:100%;margin-top:4px;font-size:0.8rem;color:#dc3545;font-weight:500;";
            ...
            $.each(errors, function (_, e) {
                ...
                if ($field.is(":radio")) {
                    $target = $form
                        .find('[name="' + e.field + '"]')
                        .first()
                        .closest('.form-check, [class*="col-"]');
                }
                // For hidden fields (amount), target the visible display input
                if (
                    $field.is(":hidden") &&
                    $field.siblings(".shf-amount-input").length
                ) {
                ...
                var $feedback = $(
                    '<div class="shf-validation-error" style="' +
                        errStyle +
                        '">' +
                        e.message +
                        "</div>",
            ...
            if (
                $first.is(":hidden") &&
                $first.siblings(".shf-amount-input").length
            ) {
    ...
    $(document).on("input change", ".is-invalid", function () {
        ...
        $el.closest(".shf-amount-wrap")
            .siblings(".shf-validation-error")
            .remove();
    });
    ...
    if (!CSS.supports("field-sizing", "content")) {
        ...
        $(document).on(
            "input",
            "textarea.shf-input, textarea.shf-input-sm",
            function () {
                autoExpand(this);
            },
        );
        $("textarea.shf-input, textarea.shf-input-sm").each(function () {
            autoExpand(this);
        });
    }
    ...
    $(document).on("submit", ".shf-confirm-delete", function (e) {
        ...
        Swal.fire({
            ...
            text:
                $(form).data("confirm-text") || "This action cannot be undone.",
            icon: "warning",
    ...
    SHF.numberToWordsEn = function (num) {
        ...
        function seg(n) {
            var r = "";
            if (n >= 10000000) {
                r += seg(Math.floor(n / 10000000)) + " Crore ";
                n %= 10000000;
            }
            if (n >= 100000) {
                r += tw(Math.floor(n / 100000)) + " Lakh ";
                n %= 100000;
            }
            if (n >= 1000) {
                r += tw(Math.floor(n / 1000)) + " Thousand ";
                n %= 1000;
            }
            if (n > 0) r += th(n);
            return r.trim();
    ...
    SHF.numberToWordsGu = function (num) {
        ...
        function seg(n) {
            var r = "";
            if (n >= 10000000) {
                r += seg(Math.floor(n / 10000000)) + " કરોડ ";
                n %= 10000000;
            }
            if (n >= 100000) {
                r += tw(Math.floor(n / 100000)) + " લાખ ";
                n %= 100000;
            }
            if (n >= 1000) {
                r += tw(Math.floor(n / 1000)) + " હજાર ";
                n %= 1000;
            }
            if (n > 0) r += th(n);
            return r.trim();
        ...
        $(".shf-amount-input").each(function () {
            ...
            function update() {
                ...
                if (isNaN(raw) || $input.val().trim() === "") {
                    ...
                    if ($words.length)
                        $words.text(
                            raw > 0 ? SHF.bilingualAmountWords(raw) : "",
                        );
                }
            }
    ...
    // FAB wiring lives in the layout (newtheme/layouts/app.blade.php) so it is
    // bound exactly once on every page. Binding it here too double-bound the
    // main button on pages that also load shf-app.js (quotation create/edit,
    // disbursement), cancelling every toggle. Left intentionally empty.

    // ─────────────────────────────────────────────────────────────
    // Global uppercase-as-you-type. Mirrors the handler in
    // public/newtheme/assets/shf-newtheme.js. Pages that load BOTH
    // scripts (quotation create, etc.) install only one handler via
    // the `__shfUppercaseInstalled` guard so events don't double-fire.
    //
    // Skip set, email-handling rationale, and caret-preservation are
    // identical to shf-newtheme.js — see that file's header comment
    // for the why.
    // ─────────────────────────────────────────────────────────────
    if (!window.__shfUppercaseInstalled) {
        ...
        function shouldSkip(el) {
            ...
            while (node && node.nodeType === 1) {
                if (
                    node.hasAttribute &&
                    node.hasAttribute("data-no-uppercase")
                ) {
                    return true;
                }
            ...
            if (el.tagName === "INPUT") {
                ...
                if (SKIP_INPUT_TYPES.indexOf(t) !== -1) {
                    return true;
                }
                ...
                if (ac.indexOf("password") !== -1 || nameId.indexOf("password") !== -1) {
                    return true;
                }
            }
            for (var i = 0; i < SKIP_CLASSES.length; i++) {
                if (el.classList && el.classList.contains(SKIP_CLASSES[i])) {
                    return true;
                }
            }
            return false;
