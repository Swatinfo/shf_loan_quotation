...
(function () {
    ...
    function readFilters() {
        return {
            ...
            created_by: canViewAll
                ? (document.getElementById("qxCreatedBy") || {}).value || ""
                : "",
        };
    }
    ...
    function buildQuery() {
        ...
        Object.keys(f).forEach(function (k) {
            if (k !== "search" && f[k] !== "") params.set(k, f[k]);
        });
    ...
    function renderRows(data) {
        if (!data.length) {
            rowsEl.innerHTML =
                '<div class="qx-loader" style="padding:36px 24px;"><div style="font-size:30px;margin-bottom:6px;">📄</div><div style="font-weight:600;color:var(--ink-3);">No quotations match the filters</div></div>';
            return;
        }
                ...
                var typeColor =
                    {
                        ...
                        pvt_ltd: "orange",
                        salaried: "violet",
                    }[q.customer_type] || "gray";

                var bankChips = (q.banks || [])
                    .map(function (n) {
                ...
                return (
                    ...
                    (q.referral_name || q.referral_type_label
                        ? '<div class="text-xs text-muted">Ref: ' +
                          ...
                          (q.referral_type_label
                              ? " (" + escapeHtml(q.referral_type_label) + ")"
                              : "") +
                          "</div>"
                        : "") +
                    (q.location_name
                        ? '<div class="text-xs text-muted">' +
                          escapeHtml(q.location_name) +
                          "</div>"
                        : "") +
                    "</td>" +
                    '<td data-label="Type"><span class="badge sq ' +
                    typeColor +
                    '">' +
                    escapeHtml(typeLabel) +
                    "</span></td>" +
                    '<td class="num tnum" data-label="Amount"><strong>' +
                    escapeHtml(q.formatted_amount) +
                    "</strong></td>" +
                    '<td data-label="Banks"><div class="qx-bank-chips">' +
                    bankChips +
                    "</div></td>" +
                    '<td data-label="Status">' +
                    statusBadge +
                    (q.is_converted
                        ? '<div class="text-xs text-muted mt-1">→ Loan ' +
                          (q.loan_status_label || "") +
                          "</div>"
                        : "") +
                    "</td>" +
                    (canViewAll
                        ? '<td data-label="Created By">' +
                          escapeHtml(q.created_by || "—") +
                          "</td>"
                        : "") +
                    '<td data-label="Date"><div class="text-xs">' +
                    escapeHtml(q.date) +
                    "</div></td>" +
                    '<td class="col-actions" data-label="Actions" onclick="event.stopPropagation();">' +
                    actions +
                    "</td>" +
                    "</tr>"
                );
    ...
    function buildRowActions(q) {
        ...
        if (q.download_branded_url)
            items.push({
                kind: "a",
                href: q.download_branded_url,
                label: "Download (Branded)",
                tone: "accent",
                icon: "M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1M12 12V4m0 8l-4-4m4 4l4-4",
            });
        if (q.download_plain_url)
            items.push({
                kind: "a",
                href: q.download_plain_url,
                label: "Download (Plain)",
                tone: "gray",
                icon: "M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3",
            });
        // Rupee glyphs for the convert / open-loan actions. SVG <text> keeps
        // these drop-in compatible with the row icon renderer below — no
        // separate code path. Single ₹ = convert (one rupee = one loan), double
        // ₹₹ = open the loan that already exists.
        var rupeeIcon =
            '<svg class="i" viewBox="0 0 24 24" fill="currentColor"><text x="12" y="17" text-anchor="middle" font-size="25" font-weight="700" font-family="Archivo, sans-serif">₹</text></svg>';
        var doubleRupeeIcon =
            '<svg class="i" viewBox="0 0 24 24" fill="currentColor"><text x="12" y="17" text-anchor="middle" font-size="25" font-weight="700" font-family="Archivo, sans-serif">₹₹</text></svg>';
        ...
        if (q.loan_url)
            items.push({
                ...
                label: "Open Loan",
                tone: "info",
                iconHtml: doubleRupeeIcon,
            });
        if (q.hold_url)
            items.push({
                kind: "a",
                href: q.show_url + "?action=hold",
                label: "Put on Hold",
                tone: "warning",
        ...
        return (
            ...
            items
                .map(function (it) {
                    var icon = it.iconHtml
                        ? it.iconHtml
                        : '<svg class="i" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="' +
                          it.icon +
                          '"/></svg>';
    ...
    function wireRowMenus() {
        ...
        rowsEl
            .querySelectorAll(".qx-actions form[data-confirm]")
            .forEach(function (f) {
                if (f.dataset.shfBound) {
                    return;
                }
                f.dataset.shfBound = "1";
                f.addEventListener("submit", function (ev) {
                    if (!confirm(f.dataset.confirm || "Are you sure?")) {
                        ev.preventDefault();
                    }
                });
            });
    ...
    function renderPager(total, filtered) {
        ...
        html +=
            '<button type="button" class="qx-pg-btn" data-page="prev"' +
            (current === 1 ? " disabled" : "") +
            ">‹</button>";
        var maxBtns = 5;
        ...
        for (var p = startP; p <= endP; p++) {
            html +=
                '<button type="button" class="qx-pg-btn ' +
                (p === current ? "active" : "") +
                '" data-page="' +
                p +
                '">' +
        ...
        html +=
            '<button type="button" class="qx-pg-btn" data-page="next"' +
    ...
    function load() {
        ...
        fetch(URLS.dataUrl + "?" + buildQuery(), {
            ...
            .catch(function () {
                rowsEl.innerHTML =
                    '<div class="qx-loader" style="color:var(--red);">Failed to load. Please try again.</div>';
            });
    }

    /* ======= Filter wiring ======= */
    var debounce = null;
    document.getElementById("qxSearch") &&
        document
            .getElementById("qxSearch")
    ...
    ["qxType", "qxStatus", "qxLoanStatus", "qxCreatedBy"].forEach(
        function (id) {
    ...
    document.getElementById("qxFilter").addEventListener("click", function () {
        state.start = 0;
        load();
    });
