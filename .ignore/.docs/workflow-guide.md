...
The system uses a **unified role model** — all roles are managed in a single `roles` table. Users can have one or more roles.

| Role | Slug | Description |
|------|------|-------------|
| **Super Admin** | `super_admin` | Full access to everything. Cannot be restricted. |
| **Admin** | `admin` | System administration, settings, user management. |
| **Branch Manager** | `branch_manager` | Oversees loans at a branch level, quotations, loan stages. |
| **BDO** | `bdo` | Business Development Officer — same access as Branch Manager. |
| **Loan Advisor** | `loan_advisor` | Primary loan handler, creates quotations, manages documents, coordinates stages. |
| **Bank Employee** | `bank_employee` | Handles bank-side processing (BSM/OSV, sanction, e-sign). |
| **Office Employee** | `office_employee` | Handles valuations, docket review, OTC clearance, office operations. |
...
|--------|-------------------|---------------------|
| View loan list | `view_loans` | All roles |
| View all loans (not just own) | `view_all_loans` | Admin, Branch Manager, BDO |
| Create a new loan | `create_loan` | Admin, Branch Manager, BDO, Loan Advisor |
| Convert quotation to loan | `convert_to_loan` | Admin, Branch Manager, BDO, Loan Advisor |
| Edit loan details | `edit_loan` | Admin, Branch Manager, BDO, Loan Advisor, Office Employee |
| Delete a loan | `delete_loan` | Admin |
| Manage loan documents | `manage_loan_documents` | Admin, Branch Manager, BDO, Loan Advisor, Office Employee |
| Manage stage status | `manage_loan_stages` | Admin, Branch Manager, BDO, Loan Advisor, Office Employee |
| Transfer stages | `transfer_loan_stages` | Admin, Branch Manager, BDO, Loan Advisor, Office Employee |
| Reject a loan | `reject_loan` | Admin, Branch Manager, BDO, Loan Advisor, Office Employee |
| Add remarks | `add_remarks` | All roles |
| Configure workflow settings | `manage_workflow_config` | Admin |

### Who Can Create Loans

Only users with the `create_loan` permission AND one of these roles:
- Super Admin
- Admin
- Branch Manager
- BDO
