...
| role | enum | yes | null | Deprecated — not queried by application |
| is_active | boolean | no | true | |
| created_by | FK → users | yes | null | who created this user |
| phone | string | yes | null | |
| task_role | string | yes | null | Deprecated — not queried by application |
...
**Note**: User roles are now managed via the `roles` / `role_user` pivot tables (unified role system). The `role` and `task_role` columns are legacy and not read by the application.
...
|--------|------|----------|---------|-------|
| id | bigint PK | no | auto | |
| name | string | no | | Display name (e.g., "Branch Manager") |
| slug | string | no | | unique (e.g., "branch_manager") |
...
**Seeded roles (7)**: super_admin, admin, branch_manager, bdo, loan_advisor, bank_employee, office_employee
...
| role_id | FK → roles | no | cascade delete |

**Indexes**: primary key on `(user_id, role_id)`
...
**Note**: This table is superseded by `role_permission` (pivot between `roles` and `permissions` tables). The legacy table remains in the database but is no longer used by `PermissionService`.
