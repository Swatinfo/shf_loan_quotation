...
|--------|------|---------|------------|
| `roles()` | BelongsToMany | Role | via `role_user` pivot — **primary role assignment** |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `isTaskRole(string $role)` | bool | @deprecated — use `hasRole()` instead |
| `hasTaskRole()` | bool | @deprecated — use `hasAnyRole(advisorEligibleRoles())` instead |
...
| `getTaskRoleLabelAttribute()` | string | @deprecated — first non-system role name (backward compat) |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `gujaratiLabels()` | array | Map of slug → Gujarati label for all 7 roles |
