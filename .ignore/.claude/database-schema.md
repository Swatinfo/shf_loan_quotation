...
| role | enum | Deprecated — not queried by application |
| is_active | boolean | default: true |
| created_by | FK -> users | nullable, set null on delete |
| phone | varchar(20) | nullable |
| task_role | string | Deprecated — not queried by application |
...
**Note**: Roles are managed via `roles` / `role_user` tables. The `role` and `task_role` columns are legacy.
...
| can_be_advisor | boolean | default: false -- appears in advisor dropdowns |
| is_system | boolean | default: false -- cannot be deleted via UI |
| timestamps | | |

**Seeded (7)**: super_admin, admin, branch_manager, bdo, loan_advisor, bank_employee, office_employee
...
### task_role_permissions (deprecated)
| Column | Type | Notes |
|--------|------|-------|
| id | bigint PK | auto-increment |
...
| sanction_decision | Loan Sanction Decision | 4 | sequential | parallel_processing |
...
| is_sanctioned | boolean | default: false — set true when sanction_decision stage approves |
...
## All 48 Permissions (6 groups)

| Slug | Group | Admin | Branch Mgr / BDO | Loan Advisor | Bank Emp | Office Emp |
|------|-------|-------|-------------------|-------------|---------|------------|
| view_settings | Settings | yes | no | no | no | no |
| edit_company_info | Settings | yes | no | no | no | no |
| edit_banks | Settings | yes | no | no | no | no |
| edit_tenures | Settings | yes | no | no | no | no |
| edit_documents | Settings | yes | no | no | no | no |
| edit_charges | Settings | yes | no | no | no | no |
| edit_services | Settings | yes | no | no | no | no |
| edit_gst | Settings | yes | no | no | no | no |
| create_quotation | Quotations | yes | yes | yes | no | no |
| generate_pdf | Quotations | yes | yes | yes | no | no |
| view_own_quotations | Quotations | yes | yes | yes | no | no |
| view_all_quotations | Quotations | yes | yes | no | no | no |
| download_pdf | Quotations | yes | yes | yes | no | no |
| delete_quotations | Quotations | yes | no | no | no | no |
| view_users | Users | yes | yes | no | no | no |
| create_users | Users | yes | no | no | no | no |
| edit_users | Users | yes | no | no | no | no |
| delete_users | Users | no | no | no | no | no |
| assign_roles | Users | yes | no | no | no | no |
| convert_to_loan | Loans | yes | yes | yes | no | no |
| view_loans | Loans | yes | yes | yes | yes | yes |
| view_all_loans | Loans | yes | yes | no | no | no |
| create_loan | Loans | yes | yes | yes | no | no |
| edit_loan | Loans | yes | yes | yes | no | yes |
| delete_loan | Loans | yes | no | no | no | no |
| manage_loan_documents | Loans | yes | yes | yes | no | yes |
| upload_loan_documents | Loans | yes | yes | yes | no | yes |
| download_loan_documents | Loans | yes | yes | yes | yes | yes |
| delete_loan_files | Loans | yes | yes | no | no | no |
| manage_loan_stages | Loans | yes | yes | yes | no | yes |
| add_remarks | Loans | yes | yes | yes | yes | yes |
| manage_workflow_config | Loans | yes | no | no | no | no |
| transfer_loan_stages | Loans | yes | yes | yes | no | yes |
| reject_loan | Loans | yes | yes | yes | no | yes |
| change_loan_status | Loans | yes | yes | yes | no | yes |
| view_loan_timeline | Loans | yes | yes | yes | yes | yes |
| manage_disbursement | Loans | yes | yes | yes | no | no |
| manage_valuation | Loans | yes | yes | no | no | yes |
| raise_query | Loans | yes | yes | yes | yes | yes |
| resolve_query | Loans | yes | yes | yes | no | yes |
| manage_customers | Customers | yes | yes | yes | no | yes |
| view_customers | Customers | yes | yes | yes | yes | yes |
| change_own_password | System | yes | yes | yes | yes | yes |
| manage_permissions | System | no | no | no | no | no |
| view_activity_log | System | yes | yes | no | no | no |
| impersonate_users | System | no | no | no | no | no |
| view_dashboard | System | yes | yes | yes | yes | yes |
| manage_notifications | System | yes | yes | yes | yes | yes |

*super_admin always has all 48 permissions (bypass in PermissionService)*
*BDO has same permissions as Branch Manager*
*manage_permissions and impersonate_users are super_admin-only by default*

*Permission groups: Settings, Quotations, Users, Loans, Customers, System (6 groups)*

---

## Unified Roles (7 roles via `roles` table)
...
| `bank_employee` | Bank-side loan processing only (linked to bank via task_bank_id) |
