...
class ManagementReportTest extends TestCase
{
    ...
    public function test_access_denied_for_non_report_roles(): void
    {
        $advisor = $this->makeUser('loan_advisor');
        $this->actingAs($advisor)->get(route('reports.management'))->assertForbidden();
        $this->actingAs($advisor)->getJson(route('reports.management.data'))->assertForbidden();
    }
    ...
    public function test_funnel_counts_amounts_percentages_and_avg_days(): void
    {
        ...
        $this->completeStage($loan, 'sanction', now()->subDays(17)->toDateTimeString(), now()->subDays(13)->toDateTimeString());
        $this->completeStage($loan, 'disbursement', now()->subDays(12)->toDateTimeString(), now()->subDays(10)->toDateTimeString());
        ...
        $this->assertSame(1, $funnel['disbursed']['count']);
        $this->assertEquals(3, $funnel['disbursed']['avg_days']);   // sanction -> disbursement
        $this->assertStringContainsString('9,00,000', $funnel['sanctioned']['amount']);
        $this->assertStringContainsString('8,50,000', $funnel['disbursed']['amount']);
    }
    ...
    public function test_trend_buckets_current_month(): void
    {
        $admin = $this->makeUser('admin');
        $advisor = $this->makeUser('loan_advisor');
        $this->makeLoan($advisor, $this->makeBranch());
    ...
    public function test_branch_manager_sees_only_own_branch_even_with_forged_branch_id(): void
    {
        $bm = $this->makeUser('branch_manager');
        $advisor = $this->makeUser('loan_advisor');
        $ownBranch = $this->makeBranch();
        $foreignBranch = $this->makeBranch();
        $bm->branches()->attach($ownBranch->id);
        $this->makeLoan($advisor, $ownBranch);
        $this->makeLoan($advisor, $foreignBranch);

        $resp = $this->actingAs($bm)
            ->getJson(route('reports.management.data', ['branch_id' => $foreignBranch->id]))
            ->assertOk();

        // Forged foreign branch intersects to nothing.
        $this->assertSame(0, $resp->json('funnel.converted.count'));

        $resp = $this->actingAs($bm)->getJson(route('reports.management.data'))->assertOk();
