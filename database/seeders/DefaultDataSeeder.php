...
class DefaultDataSeeder extends Seeder
{
    ...
    private function seedStages(): void
    {
        $stages = [
            ...
            ['id' => 18, 'stage_key' => 'original_document_verification', 'is_enabled' => true, 'stage_name_en' => 'Original Document Verification', 'stage_name_gu' => 'મૂળ દસ્તાવેજ ચકાસણી', 'sequence_order' => 4, 'is_parallel' => false, 'parent_stage_key' => 'parallel_processing', 'stage_type' => 'sequential', 'description_en' => 'Verify physical original documents collected from customer (post legal verification).', 'description_gu' => null, 'default_role' => '["branch_manager","loan_advisor","office_employee"]', 'assigned_role' => 'task_owner', 'sub_actions' => null],
    ...
    private function seedSampleQuotationAndLoan(): void
    {
        ...
        $config = app(ConfigService::class)->load();
        $quotationService = app(QuotationService::class);
        $conversionService = app(LoanConversionService::class);
        ...
        foreach ($products as $product) {
            ...
            $charges = BankCharge::where('bank_name', $product->bank_name)->first();
    ...
    private function advanceSampleLoans(): void
    {
        ...
        $loans = LoanDetail::with(['stageAssignments', 'bank', 'product'])
            ->where('status', 'active')
            ->orderBy('id')
            ->get();
     ...
     */
    private function walkLoanToStage(LoanDetail $loan, string $target, LoanStageService $stageService): void
    {
        $advisor = User::find($loan->assigned_advisor ?? $loan->created_by);
        if ($advisor) {
            auth()->login($advisor);
        }
     ...
     */
    private function completeStageWithData(LoanDetail $loan, string $stageKey, LoanStageService $stageService, string $appNumber): void
    {
        ...
        switch ($stageKey) {
            ...
            case 'rate_pf':
                $charges = BankCharge::where('bank_name', $loan->bank?->name)->first();
            ...
            case 'disbursement':
                ...
                DisbursementDetail::updateOrCreate(
                    ['loan_id' => $loan->id],
                    [
                        'mode' => 'transfer',
                        'amount' => $loan->loan_amount,
    ...
    private function seedExtraPendingQuotations(): void
    {
        ...
        foreach ($extras as $i => $row) {
            ...
            $charges = BankCharge::where('bank_name', $row['bank'])->first();
            $roi = match ($row['bank']) {
                'HDFC Bank' => ['min' => 8.75, 'max' => 9.00],
                'ICICI Bank' => ['min' => 9.00, 'max' => 9.15],
                'Axis Bank' => ['min' => 9.25, 'max' => 9.50],
    ...
    private function seedGeneralTaskSamples(): void
    {
        ...
        $loanIds = LoanDetail::pluck('id')->all();
        $i = 0;
        ...
        foreach ($users as $user) {
            ...
            foreach ($samples as $s) {
                ...
                GeneralTask::create(array_merge([
                    'title' => $title,
        ...
        $this->command?->line('  + General tasks seeded: '.GeneralTask::count().' across '.$users->count().' users.');
    }
    ...
    private function seedDvrSamples(): void
    {
        ...
        $this->command?->line('  + DVR samples seeded: '.DailyVisitReport::count().' across '.count($userIds).' field users.');
    }
}
