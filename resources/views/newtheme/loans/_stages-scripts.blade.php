        ...
        $(function() {
            ...
            $(document).on('click', '.shf-stage-transfer-btn', function() {
                ...
                if (!userId) {
                    ...
                    $select.after(
                        '<div class="shf-field-error text-danger shf-text-xs mt-1">Please select a user to transfer to</div>'
                    );
                    return;
                }
            ...
            $(document).on('click', '.shf-legal-action', function() {
                ...
                // complete_skip_bank doesn't go through bank verification at
                // all — the legal advisor field is irrelevant there. The
                // other two actions still require a name.
                if (action !== 'complete_skip_bank' && (!advisorName || !advisorName.trim())) {
                    var $input = $('#legalAdvisorName');
                    $input.addClass('is-invalid');
                    $input.next('.shf-field-error').remove();
                    $input.after(
                        '<div class="shf-field-error text-danger shf-text-xs mt-1">Legal Advisor name is required</div>'
                    );
                ...
                var confirmMsg = action === 'send_to_bank' ? 'Send for legal verification?' :
                    ...
                    'Complete legal verification without sending to bank?\n\nOriginal Document Verification will be auto-completed.' :
            ...
            function recalcPfTotals(context) {
                ...
                // Admin charges
                var adminCharges = parseFloat($card.find('input[name="admin_charges_input"]').val()) || 0;
            ...
            $(document).on('input', '#admin_charges_input', function() {
                // var $raw = $(this).closest('.shf-amount-wrap').find('.shf-amount-raw');
                // if ($raw.attr('name') === 'admin_charges_input') {
                //     console.log($raw.attr('name'));
                recalcPfTotals(this);
                // }
            });
            ...
            function calcSanctionEmi($container) {
                ...
                var $emiDisplay = $container.find('[name="emi_amount"]').closest('.shf-amount-wrap').find(
                    '.shf-amount-input');
            ...
            function validateSanctionEmi($container) {
                ...
                if (sanctionedAmount > 0 && emiAmount > sanctionedAmount) {
                    ...
                    $col.append(
                        '<div class="shf-emi-error text-danger shf-text-xs mt-1">EMI cannot exceed sanctioned amount (₹ ' +
                        SHF.formatIndianNumber(sanctionedAmount) + ')</div>');
                    return false;
                }
                return true;
            }
            ...
            $(document).on('input change', '.shf-amount-input', function() {
                ...
                if ($form.find('[name="tenure_months"]').length) {
                    var fieldName = $(this).closest('.shf-amount-wrap').find('.shf-amount-raw').attr(
                        'name');
            ...
            $(document).on('submit', '.shf-stage-notes-form', function(e) {
                var $form = $(this);
                if ($form.find('[name="tenure_months"]').length && $form.find('[name="sanctioned_amount"]')
                    .length) {
                    if (!validateSanctionEmi($form)) {
                        e.preventDefault();
                        e.stopImmediatePropagation();
                        var $emiDisplay = $form.find('[name="emi_amount"]').closest('.shf-amount-wrap')
                            .find('.shf-amount-input');
                        if ($emiDisplay.length) $emiDisplay[0].scrollIntoView({
                            behavior: 'smooth',
                            block: 'center'
                        });
                        return false;
            ...
            $('#submitQueryBtn').on('click', function() {
                ...
                if (!queryText || !queryText.trim()) {
                    ...
                    $qt.after(
                        '<div class="shf-field-error text-danger shf-text-xs mt-1">Query text is required</div>'
                    );
                    $qt.focus();
                    return;
                }
            ...
            $(document).on('submit', '.shf-query-respond', function(e) {
                ...
                if (!responseText || !responseText.trim()) {
                    ...
                    $rt.after(
                        '<div class="shf-field-error text-danger shf-text-xs mt-1">Response text is required</div>'
                    );
                    $rt.focus();
                    return;
                }
            ...
            $(document).on('input', '.shf-decision-remarks', function() {
            ...
            // Decision-ladder actions — shared by sanction_decision + technical_valuation.
            $(document).on('click', '.shf-decision-action', function() {
                var $btn = $(this);
                var loanId = $btn.data('loan-id');
                var action = $btn.data('action');
                var stage = $btn.data('stage') || 'sanction_decision';
                var $card = $btn.closest('.card-body, .card');
                var remarks = $card.find('.shf-decision-remarks').val() || '';
                var transferTo = '';

                // Route per stage: sanction uses /action, technical_valuation uses /decision.
                var url = stage === 'technical_valuation' ?
                    ('/loans/' + loanId + '/stages/technical_valuation/decision') :
                    ('/loans/' + loanId + '/stages/sanction_decision/action');

                if (action === 'reject') {
                    if (!remarks || remarks.trim().length < 10) {
                        var $textarea = $card.find('.shf-decision-remarks');
                ...
                // Approve action (sanction_decision only) — confirm before proceeding
                if (action === 'approve') {
                    Swal.fire({
                        title: 'Approve Loan?',
            ...
            $(document).on('click', '.shf-otc-transfer', function() {
                ...
                var $otcSelect = $btn.closest('.card-body').find(
                    '.shf-transfer-user[data-stage="otc_clearance"]');
                if (!userId) {
                    $otcSelect.addClass('is-invalid');
                    $otcSelect.next('.shf-field-error').remove();
                    $otcSelect.after(
                        '<div class="shf-field-error text-danger shf-text-xs mt-1">Please select an employee</div>'
                    );
                    return;
                }
