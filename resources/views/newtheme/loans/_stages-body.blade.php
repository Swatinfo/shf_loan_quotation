                ...
                @php
                    ...
                    foreach ($mainStages->values() as $idx => $sa) {
                        ...
                        if (!$tempPastCurrent) {
                            if ($idx > 0) {
                                $completedBeats++;
                            } // connector (appear+fill merged)
                            $completedBeats++; // dot beat
                        }
                        if ($isCur || ($sa->status === 'in_progress' && !$isCur)) {
                            $tempPastCurrent = true;
                        }
                    }
                        ...
                        <div class="card mb-0 border-start border-3 {{ match ($assignment->status) {
                            ...
                            <div class="card-body py-2">
                                ...
                                @php
                                    ...
                                    $getPhaseRole = fn(int $phaseIdx) => $stageWf['phases'][(string) $phaseIdx][
                                        'role'
                                    ] ?? 'task_owner';
                                    $stageSubActions = \App\Models\Stage::where(
                                        'stage_key',
                                        $assignment->stage_key,
                                    )->value('sub_actions');
                                    $stageSubActions = is_string($stageSubActions)
                                        ? json_decode($stageSubActions, true)
                                        : $stageSubActions;
                                    $phaseConfig = null;
                                    ...
                                    if (is_array($stageSubActions) && count($stageSubActions) > 1) {
                                        ...
                                        foreach ($stageSubActions as $idx => $sa) {
                                            $phaseRole =
                                                $stageWf['phases'][(string) $idx]['role'] ??
                                                ($sa['role'] ?? 'task_owner');
                                            $phaseUserId = $stageWf['phases'][(string) $idx]['default_user_id'] ?? null;
                                            $phaseUserName = $phaseUserId
                                                ? \App\Models\User::find($phaseUserId)?->name ?? ''
                                                : '';
                                            ...
                                            $phases[] = [
                                                ...
                                                'role' =>
                                                    $roleLabelsMap[$phaseRole] ??
                                                    ucwords(str_replace('_', ' ', $phaseRole)),
                                                'user' => $phaseUserName,
                                            ];
                                        }
                                    ...
                                    <div class="d-flex align-items-center gap-2 mt-2 mb-1 flex-wrap">
                                        @foreach ($phaseConfig['phases'] as $pi => $phase)
                                            @php
                                                $isCurrent =
                                                    $assignment->status === 'in_progress' &&
                                                    $phaseConfig['current'] === $phase['key'];
                                                $isDone =
                                                    $assignment->status === 'completed' ||
                                                    (int) $phaseConfig['current'] > (int) $phase['key'];
                                                $phaseUserLabel = !empty($phase['user'])
                                                    ? $phase['user']
                                                    : $phase['role'];
                                            @endphp
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                @elseif($isCurrent)
                                                    <span
                                                        ...
                                                        <svg class="shf-icon-sm" fill="currentColor"
                                                            viewBox="0 0 24 24">
                                                        ...
                                                        @if ($sub->status === 'completed')
                                                            ...
                                                            <div class="mt-2 border-top pt-2 shf-stage-saved-data"
                                                                ...
                                                                @switch($sub->stage_key)
                                                                    ...
                                                                    @case('legal_verification')
                                                                        ...
                                                                        @endif
                                                                        @if (!empty($notesData['legal_skipped_bank']))
                                                                            <div class="small text-warning">Legal verification
                                                                                waved — completed by task owner</div>
                                                                        @else
                                                                    ...
                                                                    @case('original_document_verification')
                                                                        @if (!empty($notesData['auto_completed_reason']) && $notesData['auto_completed_reason'] === 'legal_skipped_bank')
                                                                            <div class="small text-muted">Auto-completed
                                                                                because
                                                                                legal verification waved.</div>
                                                                        @else
                                                                            @if (!empty($notesData['verification_date']))
                                                                                <div class="small"><span
                                                                                        class="text-muted">Verified
                                                                                        On:</span>
                                                                                    <strong>{{ $notesData['verification_date'] }}</strong>
                                                                                </div>
                                                                            @endif
                                                                            @if (!empty($notesData['remarks']))
                                                                                <div class="small text-muted mt-1">
                                                                                    {{ $notesData['remarks'] }}
                                                                                </div>
                                                                            @endif
                                                            ...
                                                            <div id="edit-{{ $sub->stage_key }}"
                                                                ...
                                                                @switch($sub->stage_key)
                                                                    ...
                                                                    @case('property_valuation')
                                                                        <div
                                                                            class="mt-2 border-top pt-2 d-flex gap-2 flex-wrap">
                                                            ...
                                                            @switch($sub->stage_key)
                                                                ...
                                                                @case('bsm_osv')
                                                                    <div class="mt-2 border-top pt-2">
                                                                        <small class="text-muted d-block mb-2">Assigned to bank
                                                                            employee for BSM/OSV verification. Mark complete
                                                                            when
                                                                            done.</small>
                                                                ...
                                                                @case('legal_verification')
                                                                    ...
                                                                    @if ($legalPhase === '1')
                                                                        ...
                                                                        @php
                                                                            $legalP2Role =
                                                                                $wfConfig['legal_verification'][
                                                                                    'phases'
                                                                                ]['1']['role'] ?? 'bank_employee';
                                                                            // Same authority as `LoanStageController::canSkipLegalBank()` —
                                                                            // creator / advisor / branch_manager-or-bdh of the loan's branch.
$authUser = auth()->user();
$canSkipLegalBank =
    $authUser &&
            ...
            $authUser
                ...
                ->where(
                    'branches.id',
                                                                        ...
                                                                        @endphp
                                                                        <div class="mt-2 border-top pt-2">
                                                                            <small class="text-muted d-block mb-2">Enter
                                                                                suggested
                                                                                legal advisor name and send to
                                                                                {{ str_replace('_', ' ', $legalP2Role) }}.</small>
                                                                            <input type="text"
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                <select
                                                                                    ...
                                                                                    <option value="">Select
                                                                                        {{ ucwords(str_replace('_', ' ', $legalP2Role)) }}...
                                                                                    </option>
                                                                                </select>
                                                                            ...
                                                                            @if ($canSkipLegalBank)
                                                                                <div class="mt-2">
                                                                                    <small class="text-muted d-block mb-1">
                                                                                        Or complete legal verification without
                                                                                        sending to
                                                                                        {{ str_replace('_', ' ', $legalP2Role) }}.
                                                                                        Original Document Verification will be
                                                                                        marked complete automatically.
                                                                                    </small>
                                                                                    <button
                                                                                        class="btn-accent-outline btn-accent-sm shf-legal-action"
                                                                                        data-loan-id="{{ $loan->id }}"
                                                                                        data-action="complete_skip_bank">
                                                                                        <svg class="shf-icon-2xs"
                                                                                            fill="none"
                                                                                            stroke="currentColor"
                                                                                            viewBox="0 0 24 24">
                                                                                            <path stroke-linecap="round"
                                                                                        ...
                                                                                        {{-- Complete without bank verification --}}
                                                                                        Wave
                                                                                    </button>
                                                                                </div>
                                                                            @endif
                                                                    ...
                                                                    @elseif($legalPhase === '2')
                                                                        ...
                                                                        <div class="mt-2 border-top pt-2">
                                                                            <div
                                                                                class="alert alert-info py-2 mb-2 shf-text-sm">
                                                                    ...
                                                                    @elseif($legalPhase === '3')
                                                                        ...
                                                                        <div class="mt-2 border-top pt-2">
                                                                            ...
                                                                            <small class="text-muted d-block mb-2">You can
                                                                                reassign
                                                                ...
                                                                @case('technical_valuation')
                                                                    @php
                                                                        ...
                                                                        $tvVal = $loan->valuationDetails
                                                                            ...
                                                                            ->first();
                                                                        // Escalation ladder (same model as sanction_decision; no Approve —
                                                                        // filling the valuation form is the approval).
                                                                        $tvEscHistory =
                                                                            $tvNotes['escalation_history'] ?? [];
                                                                        $tvUser = auth()->user();
                                                                        $tvIsBM = $tvUser->hasRole('branch_manager');
                                                                        $tvIsBDH = $tvUser->hasRole('bdh');
                                                                        $tvIsAdm = $tvUser->hasAnyRole([
                                                                            'super_admin',
                                                                            'admin',
                                                                        ]);
                                                                        $tvLastRole = !empty($tvEscHistory)
                                                                            ? $tvEscHistory[count($tvEscHistory) - 1][
                                                                                    'to_role'
                                                                                ] ?? null
                                                                            : null;
                                                                        $tvLevel =
                                                                            $tvLastRole === 'branch_manager'
                                                                                ? 'bm'
                                                                                : ($tvLastRole === 'bdh'
                                                                                    ? 'bdh'
                                                                                    : 'base');
                                                                    ...
                                                                    @if ($tvPhase === '1')
                                                                        ...
                                                                        <div class="mt-2 border-top pt-2">
                                                                            <small class="text-muted d-block mb-2">Send for
                                                                                technical valuation.</small>
                                                                            <div
                                                                                class="d-flex align-items-center gap-2 flex-wrap">
                                                                                <select
                                                                                    class="shf-input shf-input-sm shf-transfer-user"
                                                                                    ...
                                                                                    style="max-width:220px">
                                                                                    <option value="">Select
                                                                                        {{ ucwords(str_replace('_', ' ', $tvP2Role)) }}...
                                                                                    </option>
                                                                                </select>
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        {{-- Escalation ladder — no Approve (filling the form completes it) --}}
                                                                        <div class="mt-2 border-top pt-2">
                                                                            @if (!empty($tvEscHistory))
                                                                                <small
                                                                                    class="fw-semibold text-muted d-block mb-2">Escalation
                                                                                    History:</small>
                                                                                @foreach ($tvEscHistory as $esc)
                                                                                    <div
                                                                            ...
                                                                            @if (
                                                                                $tvIsAdm ||
                                                                                    $tvLevel === 'base' ||
                                                                                    ($tvLevel === 'bm' && $tvIsBM) ||
                                                                                    (in_array($tvLevel, ['bm', 'bdh']) && ($tvIsBM || $tvIsBDH)))
                                                                                <div class="mb-2">
                                                                                    <label
                                                                                        class="shf-form-label shf-text-xs">Remarks</label>
                                                                                    <textarea class="shf-input shf-input-sm shf-decision-remarks" rows="2"
                                                                                        placeholder="Required for escalation or rejection..."></textarea>
                                                                                </div>
                                                                                <div class="d-flex flex-wrap gap-2">
                                                                                    {{-- Escalate to BM — base level (or admin) --}}
                                                                                    @if ($tvIsAdm || $tvLevel === 'base')
                                                                                        <button
                                                                                            class="btn-accent-sm shf-btn-warning shf-decision-action"
                                                                                            data-loan-id="{{ $loan->id }}"
                                                                                            data-stage="technical_valuation"
                                                                                            data-action="escalate_to_bm">Escalate
                                                                                            to BM</button>
                                                                                    @endif
                                                                                    {{-- Escalate to BDH — only BM at BM level (or admin) --}}
                                                                                    @if ($tvIsAdm || ($tvLevel === 'bm' && $tvIsBM))
                                                                                        <button
                                                                                            class="btn-accent-sm shf-btn-warning shf-decision-action"
                                                                                            data-loan-id="{{ $loan->id }}"
                                                                                            data-stage="technical_valuation"
                                                                                            data-action="escalate_to_bdh">Escalate
                                                                                            to BDH</button>
                                                                                    @endif
                                                                                    {{-- Reject — BM/BDH at escalated level (or admin) --}}
                                                                                    @if ($tvIsAdm || (in_array($tvLevel, ['bm', 'bdh']) && ($tvIsBM || $tvIsBDH)))
                                                                                        <button
                                                                                            class="btn-accent-sm shf-btn-danger-alt shf-decision-action"
                                                                                            data-loan-id="{{ $loan->id }}"
                                                                                            data-stage="technical_valuation"
                                                                ...
                                                                @case('sanction_decision')
                                                                    @php
                                                                        ...
                                                                        // Escalation level from history (independent of base role).
                                                                        $sdLastRole = !empty($sdEscHistory)
                                                                            ? $sdEscHistory[count($sdEscHistory) - 1][
                                                                                    'to_role'
                                                                                ] ?? null
                                                                            : null;
                                                                        $sdLevel =
                                                                            $sdLastRole === 'branch_manager'
                                                                                ? 'bm'
                                                                                : ($sdLastRole === 'bdh'
                                                                                    ? 'bdh'
                                                                                    : 'base');
                                                                    ...
                                                                    <div class="mt-2 border-top pt-2">
                                                                        ...
                                                                        <div class="d-flex flex-wrap gap-2">
                                                                            {{-- Approve — current holder (or admin) --}}
                                                                            <button
                                                                                class="btn-accent-sm shf-btn-success shf-decision-action"
                                                                                data-loan-id="{{ $loan->id }}"
                                                                                data-stage="sanction_decision"
                                                                                data-action="approve">
                                                                                <svg class="shf-icon-2xs" fill="none"
                                                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                                                    <path stroke-linecap="round"
                                                                            ...
                                                                            {{-- Reject — BM/BDH at escalated level (or admin) --}}
                                                                            @if ($sdIsAdm || (in_array($sdLevel, ['bm', 'bdh']) && ($sdIsBM || $sdIsBDH)))
                                                                                <button
                                                                                    class="btn-accent-sm shf-btn-danger-alt shf-decision-action"
                                                                                    data-loan-id="{{ $loan->id }}"
                                                                                    data-stage="sanction_decision"
                                                        ...
                                                        @if ($subActiveQueries->isNotEmpty())
                                                            <div
                                                                class="alert alert-warning py-2 mt-2 mb-1 shf-text-sm">
                                                                ...
                                                                @foreach ($subActiveQueries as $q)
                                                                    <div class="border-top mt-2 pt-2">
                                                                        ...
                                                                        @endif
                                                                        {{-- Resolve button (raiser, current stage assignee, or admin — pending or responded) --}}
                                                                        @if (
                                                                            in_array($q->status, ['pending', 'responded'], true) &&
                                                                                ($q->raised_by === auth()->id() ||
                                                                                    $sub->assigned_to === auth()->id() ||
                                                        ...
                                                        @if ($isSubAssignee && auth()->user()->hasPermission('manage_loan_stages'))
                                                            <div
                                                                class="mt-1 d-flex gap-1 flex-wrap align-items-center">
                                ...
                                @if (
                                    $assignment->stage_key === 'otc_clearance' &&
                                        $assignment->status === 'skipped' &&
                                        $loan->disbursement?->disbursement_type === 'fund_transfer')
                                    <div class="mt-2 text-muted shf-text-sm">
                                        <svg class="shf-icon-sm shf-icon-inline" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @switch($assignment->stage_key)
                                        @case('inquiry')
                                            @if ($assignment->status === 'in_progress' || ($assignment->status === 'completed' && !$loan->isBasicEditLocked()))
                                                <div class="mt-2 border-top pt-2">
                                                    <small class="text-muted">Customer and loan details were captured during
                                                        loan
                                                        creation.</small>
                                            ...
                                            @if (
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    <small class="text-muted">Select and manage required documents for this
                                                        loan.</small>
                                                    <a href="{{ route('loans.documents', $loan) }}"
                                                        class="btn-accent-sm ms-2">
                                                        <svg class="shf-icon-2xs" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
                                            ...
                                            @if (
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    @if ($docProgress['percentage'] < 100)
                                                        <small class="text-warning ms-2">All required documents must be
                                                            collected
                                        ...
                                        @case('rate_pf')
                                            ...
                                            @if ($ratePfCompleted)
                                                <div class="mt-2 border-top pt-2 shf-stage-saved-data" id="saved-rate_pf">
                                                    <div class="row g-2">
                                                        ...
                                                        <div class="col-sm-6">
                                                            <div class="small"><span class="text-muted">Rate Offered
                                                                    Date:</span>
                                                        ...
                                                        <div class="col-sm-6">
                                                            <div class="small"><span class="text-muted">Bank
                                                                    Reference:</span>
                                                        ...
                                                        <div class="col-sm-6">
                                                            <div class="small"><span class="text-muted">Processing
                                                                    Fee:</span>
                                                    ...
                                                    @include('newtheme.loans.partials.stage-notes-form', [
                                                        'fields' => [
                                                            ...
                                                            [
                                                                ...
                                                                'type' => 'date',
                                                                'min_date' => empty(
                                                                    $assignment->getNotesData()['rate_valid_until']
                                                                )
                                                                    ? now()->format('d/m/Y')
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    <div class="d-flex align-items-center gap-2 flex-wrap mt-1">
                                                        ...
                                                        <select class="shf-input shf-input-sm shf-transfer-user"
                                                            ...
                                                            <option value="">Select
                                                                {{ ucwords(str_replace('_', ' ', $ratePfP1Role)) }}...
                                                            </option>
                                                        </select>
                                            ...
                                            @elseif($ratePfPhase === '2')
                                                <div class="mt-2 border-top pt-2">
                                                    <div class="alert alert-info py-2 mb-2 shf-text-sm">
                                                        <strong>Rate request received.</strong> Review and update all details,
                                                        then
                                            ...
                                            @elseif($ratePfPhase === '3')
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    <small class="fw-semibold text-muted d-block mb-1">Bank has reviewed.
                                                        Values
                                        ...
                                        @case('sanction')
                                                ...
                                                <div class="mt-2 border-top pt-2 shf-stage-saved-data" id="saved-sanction">
                                                    <div class="row g-2">
                                                        ...
                                                        @if (!empty($sanctionNotes['conditions']))
                                                            <div class="col-12">
                                                                <div class="small"><span
                                                                        class="text-muted">Conditions:</span>
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    <div class="d-flex align-items-center gap-2 flex-wrap">
                                                        <select class="shf-input shf-input-sm shf-transfer-user"
                                                            ...
                                                            data-loan-id="{{ $loan->id }}" style="max-width:220px">
                                                            <option value="">Select
                                                                {{ ucwords(str_replace('_', ' ', $sanctionP2Role)) }}...
                                                            </option>
                                                        </select>
                                                        <button class="btn-accent-sm shf-sanction-action"
                                                            data-loan-id="{{ $loan->id }}"
                                                            data-action="send_for_sanction">
                                            ...
                                            @elseif($sanctionPhase === '2')
                                                <div class="mt-2 border-top pt-2">
                                                    <div class="alert alert-info py-2 mb-2 shf-text-sm">
                                                        <strong>Waiting for sanction letter.</strong> Please generate the
                                                        sanction
                                        ...
                                        @case('docket')
                                            ...
                                            @if ($docketPhase === '1')
                                                ...
                                                <div class="d-flex align-items-center gap-2 flex-wrap mt-2">
                                                    <select class="shf-input shf-input-sm shf-transfer-user"
                                                        data-stage="docket" data-role="{{ $docketP2Role }}"
                                                        data-loan-id="{{ $loan->id }}" style="max-width:220px">
                                                        <option value="">Select
                                                            {{ ucwords(str_replace('_', ' ', $docketP2Role)) }}...</option>
                                                    </select>
                                            ...
                                            @elseif($docketPhase === '2')
                                                ...
                                                @else
                                                    <div class="mt-2 border-top pt-2">
                                                        <div class="row g-2">
                                                            <div class="col-sm-6">
                                                                <div class="small"><span class="text-muted">Login
                                                                        Date:</span>
                                                                    <strong>{{ $docketNotes['login_date'] ?? '—' }}</strong>
                                                                </div>
                                                            </div>
                                                            <div class="col-sm-6">
                                                                <div class="small"><span class="text-muted">Sanctioned
                                                                        Amount:</span>
                                                                    <strong>₹
                                                                        {{ $docketNotes['sanctioned_amount'] ?? '—' }}</strong>
                                                                </div>
                                                            </div>
                                                            <div class="col-sm-6">
                                                                <div class="small"><span class="text-muted">Sanctioned
                                                            ...
                                                            <div class="col-sm-6">
                                                                <div class="small"><span class="text-muted">Tenure:</span>
                                                                    <strong>{{ $docketNotes['tenure_months'] ?? '—' }}
                                                                        months</strong>
                                                                </div>
                                                            </div>
                                                            <div class="col-sm-6">
                                                                <div class="small"><span class="text-muted">EMI
                                                                        Amount:</span>
                                        ...
                                        @case('esign')
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    <div class="d-flex align-items-center gap-2 flex-wrap">
                                                        <select class="shf-input shf-input-sm shf-transfer-user"
                                                            ...
                                                            data-loan-id="{{ $loan->id }}" style="max-width:220px">
                                                            <option value="">Select
                                                                {{ ucwords(str_replace('_', ' ', $esignP2Role)) }}...</option>
                                                        </select>
                                            ...
                                            @elseif($esignPhase === '2')
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    <small class="text-muted d-block mb-2">Generate E-Sign & eNACH documents
                                                        for
                                            ...
                                            @elseif($esignPhase === '3')
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    <button class="btn-accent-sm shf-esign-action"
                                                        data-loan-id="{{ $loan->id }}"
                                                        data-action="esign_customer_done">
                                                ...
                                                <div class="mt-2 border-top pt-2">
                                                    ...
                                                    @if ($disbData->notes)
                                                        <div class="small"><span class="text-muted">Notes:</span>
                                                            {{ $disbData->notes }}</div>
                                                    @endif
                                                    <a href="{{ route('loans.disbursement', $loan) }}"
                                                        class="btn-accent-outline btn-accent-sm mt-2">
                                                        <svg class="shf-icon-2xs" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            @else
                                                <div class="mt-2">
                                                    ...
                                                    <a href="{{ route('loans.disbursement', $loan) }}"
                                                        ...
                                                        {{ !empty($disbEntries) ? 'Manage Disbursement Entries' : 'Open Disbursement Form' }}
                                                    </a>
                                                </div>
                                            @endif
                                        @break
                                ...
                                @if ($mainActiveQueries->isNotEmpty())
                                    <div class="alert alert-warning py-2 mt-2 shf-text-sm">
                                        <strong>Active Queries ({{ $mainActiveQueries->count() }})</strong> — Stage
                                        cannot
        ...
        <div class="modal-dialog">
            <div class="modal-content" style="border: none; border-radius: 12px;">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn-accent btn-accent-sm" id="submitQueryBtn"><svg class="shf-icon-md"
