<script>
    $(function() {
        ...
        function updateProgress(progress) {
            ...
            $('.shf-doc-progress-text').text(progress.resolved + '/' + progress.total + ' (' + progress
                .percentage + '%)');
        }
        ...
        function scrollToNextPending($afterRow) {
            ...
            if ($pending.length) {
                setTimeout(function() {
                    ...
                    setTimeout(function() {
                        $pending.removeClass('shf-doc-highlight');
                    }, 1500);
                }, 300);
            }
        }
        ...
        // Ignore clicks that originate from interactive controls (waive/remove/
        // delete-file buttons, upload label, download link, checkbox) — those have
        // their own delegated handlers and must not also trigger the row toggle.
        $(document).on('click', '.shf-doc-row', function(e) {
            if ($(e.target).closest(
                    '.shf-doc-actions, .shf-doc-toggle, .shf-doc-delete-file, a, button, label, input')
                .length) {
                return;
            }
        ...
        $('#addDocForm').on('submit', function(e) {
            ...
            $.post($form.attr('action'), $form.serialize() + '&_token=' + csrfToken)
                ...
                .fail(function(xhr) {
                    Swal.fire('Error', xhr.responseJSON?.error || 'Failed to add document',
                        'error');
                });
        });
    });
