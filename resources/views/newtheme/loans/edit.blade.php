                    ...
                    <div class="le-grid">
                        ...
                        <div class="le-field">
                            ...
                            <input type="text" name="pan_number" id="lePan" class="input"
                                ...
                                style="text-transform:uppercase;letter-spacing:0.06em;" autocomplete="off">
                            <small class="lbl shf-pan-lookup-hint" style="display:none;color:var(--ink-3);"></small>
                            <div class="shf-pan-loans" style="display:none;margin-top:4px;font-size:0.85rem;"></div>
                        </div>
                        <div class="le-field">
...
@push('page-scripts')
    ...
    <script>
        $(function () {
            // PAN autofill — fill empty customer fields from the latest known
            // KYC for that PAN (non-destructive: never overwrites entered values).
            var lookupUrl = @json(route('customers.lookup'));
            var $pan = $('#lePan'), $hint = $('.shf-pan-lookup-hint'), $loans = $('.shf-pan-loans');
            ...
            function renderLoans(loans) {
                ...
                var links = loans.map(function (l) {
                    return '<a href="' + l.url + '" target="_blank" rel="noopener" style="color:var(--accent);font-weight:600;">#' + esc(l.loan_number) + '</a>';
                }).join(', ');
                $loans.html('<span style="color:#b45309;">&#9888; This customer has ' + loans.length + ' open loan(s): </span>' + links).show();
            }
            $pan.on('blur change', function () {
                ...
                $.getJSON(lookupUrl, { pan: pan }).done(function (r) {
                    ...
                    var c = r.customer;
                    var fill = function (sel, v) { var $e = $(sel); if (v && !$.trim($e.val())) { $e.val(v); } };
                    fill('#leCustomerName', c.customer_name);
                    fill('#leCustomerPhone', c.mobile);
                    fill('#leCustomerEmail', c.email);
                    fill('#leDob', c.date_of_birth);
                    $hint.text('Existing customer: ' + (c.customer_name || '')).show();
                    renderLoans(r.loans);
                });
            });
        });
