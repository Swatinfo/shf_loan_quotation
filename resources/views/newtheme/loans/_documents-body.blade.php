...
// from leaking Laravel's HTML error page into the middle of this
    ...
    $docsLocked =
        $docsLocked ??
        !in_array($loan->status, ['active', 'on_hold']) ||
            $loan
                ->stageAssignments()
    ...
    <div class="px-3 px-sm-4 px-lg-5">
        ...
        @if ($docsLocked)
            <div class="alert alert-warning mb-3 shf-text-sm">
                <strong>Documents are locked.</strong> Verification stages have started — documents can no longer be
                modified.
            </div>
        @endif
        ...
        <div class="card border-0 shadow-sm mb-4" style="position:sticky; top:0; z-index:100;">
            <div class="card-body py-2">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <span class="shf-doc-progress-text">{{ $progress['resolved'] }}/{{ $progress['total'] }}
                        ({{ $progress['percentage'] }}%)</span>
                </div>
                <div class="progress shf-progress-md">
                    <div class="progress-bar bg-success shf-doc-progress-bar"
                ...
                @forelse($documents as $doc)
                    <div class="col-xl-6">
                        <div class="shf-doc-item d-flex align-items-center gap-2 {{ !$docsLocked ? 'shf-doc-row' : '' }} {{ $doc->isResolved() ? 'shf-doc-received' : ($doc->status === 'rejected' ? 'shf-doc-rejected' : 'shf-doc-pending') }}"
                            data-doc-id="{{ $doc->id }}"
                            @if (auth()->user()->hasPermission('manage_loan_documents') && !$docsLocked) data-toggle-url="{{ route('loans.documents.status', [$loan, $doc]) }}"
                             data-current-status="{{ $doc->status }}" @endif>
                            {{-- Checkbox --}}
                            ...
                            <div class="flex-grow-1" style="min-width:0;">
                                <span
                                    class="{{ $doc->isReceived() ? 'text-decoration-line-through text-muted' : '' }}">{{ $doc->document_name_en }}</span>
                                @if ($doc->is_required)
                                ...
                                @if ($doc->status === 'received')
                                    <small class="text-success shf-text-2xs d-block">
                                        <svg class="shf-icon-2xs" style="vertical-align:-1px;" fill="none"
                                ...
                                @if ($doc->hasFile())
                                    <small class="d-flex align-items-center gap-1 shf-text-2xs flex-wrap">
                                        ...
                                        <span class="text-muted">{{ $doc->file_name }}
                                            ({{ $doc->formattedFileSize() }})</span>
                                        @if (auth()->user()->hasPermission('download_loan_documents'))
                                            <a href="{{ route('loans.documents.download', [$loan, $doc]) }}"
                                                class="text-primary shf-text-2xs"
                                                onclick="event.stopPropagation();">Download</a>
                                        @endif
                                        @if (auth()->user()->hasPermission('delete_loan_files'))
                                            <button
                                                class="btn p-0 text-danger shf-doc-delete-file shf-text-2xs shf-btn-minimal"
                                                data-url="{{ route('loans.documents.deleteFile', [$loan, $doc]) }}">Delete</button>
                                        @endif
                                    </small>
                                @endif
                            </div>
                            ...
                            @if (!$docsLocked)
                                <div class="flex-shrink-0 d-flex gap-1 shf-doc-actions">
                                    @if (auth()->user()->hasPermission('upload_loan_documents'))
                                        <label class="btn-accent-sm shf-text-2xs shf-upload-label" title="Upload file"
                                            onclick="event.stopPropagation();">
                                            <svg class="shf-icon-xs shf-icon-inline" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                                            </svg> {{ $doc->hasFile() ? 'Replace' : 'Upload' }}
                                            <input type="file" class="d-none shf-doc-upload-input"
                                                data-url="{{ route('loans.documents.upload', [$loan, $doc]) }}"
                                                accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.xls,.xlsx">
                                        </label>
                                    @endif
                                    @if (auth()->user()->hasPermission('manage_loan_documents'))
                                        @if (!in_array($doc->status, ['received', 'waived']))
                                            <button class="btn-accent-sm shf-doc-action shf-text-2xs shf-btn-warning"
                                                data-url="{{ route('loans.documents.status', [$loan, $doc]) }}"
                                                data-status="waived">
                                                <svg class="shf-icon-xs shf-icon-inline" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endif
                                    @if (auth()->user()->hasPermission('delete_loan_files'))
                                        <button class="btn-accent-sm shf-doc-remove shf-text-2xs shf-btn-danger"
                                            data-url="{{ route('loans.documents.destroy', [$loan, $doc]) }}">
                                            <svg class="shf-icon-xs shf-icon-inline" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="card-body">
                    ...
                    <form id="addDocForm" action="{{ route('loans.documents.store', $loan) }}">
                        <div class="row g-2 align-items-end">
                            <div class="col-sm-5">
                                <input type="text" name="document_name_en" class="shf-input shf-input-sm"
                                    placeholder="Document name (English)" required>
                            </div>
                            <div class="col-sm-4">
                                <input type="text" name="document_name_gu" class="shf-input shf-input-sm"
                                    placeholder="Name (Gujarati) — optional">
                            </div>
                            <div class="col-sm-3">
                                <button type="submit" class="btn-accent-sm w-100"><svg class="shf-icon-2xs"
