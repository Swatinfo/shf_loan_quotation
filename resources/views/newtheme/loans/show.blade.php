...
@php
    ...
    // Customer details are shown from the per-loan KYC snapshot when present,
    // falling back to the loan's inline fields (e.g. for not-yet-backfilled loans).
    $kyc = $loan->customerKycDetails;
    $kycName = $kyc->customer_name ?? $loan->customer_name;
    $kycPhone = $kyc->mobile ?? $loan->customer_phone;
    $kycEmail = $kyc->email ?? $loan->customer_email;
    $kycPan = $kyc->pan_number ?? $loan->pan_number;
@endphp
        ...
        <div class="head-row">
            ...
            <div class="head-actions">
                ...
                @if ($appNumberDone && $canChangeDme)
                    <button type="button" class="btn" id="lsChangeDmeBtn">
                        <svg class="i" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
        ...
        <div class="card ls-card ls-collapsible collapsed" data-target="#lsDetails">
            ...
            <div class="card-bd ls-collapse-body" id="lsDetails">
                <div class="ls-kv-grid">
                    <div><span>Customer Name</span><span>{{ $kycName }}</span></div>
                    ...
                    @if ($kycPhone)
                        <div><span>Phone</span><span><a href="tel:{{ $kycPhone }}" class="ls-link">{{ $kycPhone }}</a></span></div>
                    @endif
                    @if ($kycEmail)
                        <div><span>Email</span><span><a href="mailto:{{ $kycEmail }}" class="ls-link">{{ $kycEmail }}</a></span></div>
                    @endif
                    @if ($kycPan)
                        <div><span>PAN</span><span class="ls-mono">{{ $kycPan }}</span></div>
                    @endif
...
@push('page-scripts')
    ...
    @if ($appNumberDone && $canChangeDme)
        <script>
            (function () {
                var cfg = window.__LS || {};
                var btn = document.getElementById('lsChangeDmeBtn');
                if (!btn || !cfg.dmeUpdateUrl) { return; }
                ...
                btn.addEventListener('click', function () {
                    ...
                    Swal.fire({
                        title: 'Set DME',
                        html: '<select id="swalDme" class="swal2-select" style="width:90%;">' +
                            '<option value="">-- Select DME --</option>' + opts + '</select>',
                        showCancelButton: true,
                        confirmButtonText: 'Save',
                        confirmButtonColor: '#f15a29',
                        preConfirm: function () {
                            var v = document.getElementById('swalDme').value;
                            if (!v) { Swal.showValidationMessage('Please select a DME'); return false; }
                            return v;
                        }
                    }).then(function (res) {
                        if (!res.isConfirmed) { return; }
                        if (window.SHF && SHF.loader) { SHF.loader.begin(); }
                        fetch(cfg.dmeUpdateUrl, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token, 'Accept': 'application/json' },
                            body: JSON.stringify({ dme_user_id: res.value })
                        }).then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
                            .then(function (resp) {
                                if (resp.ok && resp.j.success) {
                                    Swal.fire({ icon: 'success', title: 'DME updated', timer: 1200, showConfirmButton: false })
