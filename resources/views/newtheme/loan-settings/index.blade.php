...
@section('content')
    ...
    <main class="content loan-settings-nt">
        ...
        {{-- Flash + validation feedback — without this every save outcome (success
             or server-side rejection, e.g. overlapping payout slabs) is invisible. --}}
        @if (session('success'))
            <div class="card" style="border-left:3px solid var(--green, #27ae60); margin-bottom:14px;">
                <div class="card-bd" style="color:var(--green, #1f8c4d); padding:12px 16px;">{{ session('success') }}</div>
            </div>
        @endif
        @if (session('error'))
            <div class="card" style="border-left:3px solid var(--red, #c0392b); margin-bottom:14px;">
                <div class="card-bd" style="color:var(--red, #c0392b); padding:12px 16px;">{{ session('error') }}</div>
            </div>
        @endif
        @if ($errors->any())
            <div class="card" style="border-left:3px solid var(--red, #c0392b); margin-bottom:14px;">
                <div class="card-bd" style="color:var(--red, #c0392b); padding:12px 16px;">
