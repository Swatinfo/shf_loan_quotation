...
<body>
    ...
    @auth
        ...
        <script>
            (function() {
                ...
                // Last unread count seen by the poll. Used to detect a rise so we
                // can play the in-app chime (window.SHFPush.playChime). null until
                // the first poll establishes a baseline — so no chime on page load.
                var lastCount = null;
                    ...
                    fetch(URL, {
                        ...
                        .then(function(data) {
                            if (data && typeof data.count !== 'undefined') {
                                // New notification arrived since the last poll → chime.
                                // SHFPush honours the user's mute flag + chosen preset.
                                if (lastCount !== null && data.count > lastCount &&
                                    window.SHFPush && typeof window.SHFPush.playChime === 'function') {
        ...
        {{-- Native WebView shell bridge — registers the FCM device token (no-op in a plain browser) --}}
        <script src="{{ asset('newtheme/js/native-bridge.js') }}?v={{ $v }}"></script>
        <script>
            (function() {
                if (!window.SHFPush || !window.SHFPush.supported()) {
                    return;
                }
                // Inside the native app shell (Android/iOS WebView) notifications
                // are delivered via FCM, never Web Push — skip all subscription
                // logic so the same device isn't notified twice.
                if (/SHFApp/.test(navigator.userAgent || '')) {
                    return;
                }
        ...
        {{-- Logout interceptor — drop this device's push/FCM registration before
         the session ends so a logged-out user stops receiving notifications.
         Removes only the CURRENT device (Web Push browser sub + the native FCM
         token posted as a hidden field), leaving the user's other devices intact.
         Delegated at the document level so it covers every logout form (header
         dropdown, bottom-nav). A short guard timeout ensures cleanup never blocks
        ...
        <script>
            (function() {
                ...
                document.addEventListener('submit', function(e) {
                    var form = e.target;
                    if (!form || form.getAttribute('action') !== logoutUrl) { return; }
                    if (form.__shfLogoutCleaned) { return; }
                    e.preventDefault();
                    form.__shfLogoutCleaned = true;

                    // Carry the native FCM token so the server can drop just this
                    // device (reliable even if the async cleanup below fails).
                    if (window.shfNative && window.shfNative.token) {
                        var hidden = document.createElement('input');
                        hidden.type = 'hidden';
                        hidden.name = 'device_token';
                        hidden.value = window.shfNative.token;
                    ...
                    if (window.shfNative && window.shfNative.token) {
                        tasks.push(fetch('/api/device/unregister', {
                            method: 'POST',
                            headers: {
                            ...
                            credentials: 'same-origin',
                            body: JSON.stringify({ token: window.shfNative.token }),
                            keepalive: true,
                        }).catch(function() {}));
                    }
    ...
    <script>
        if ('serviceWorker' in navigator) {
            ...
            // When a Web Push arrives and this tab is open, the service worker
            // posts {type:'shf-play-chime'}. Play the user's chosen chime now
            // (SHFPush reads their preset + mute flag, and de-dupes against the
            // 60s badge poll so we never double-chime).
            navigator.serviceWorker.addEventListener('message', function(event) {
                if (event.data && event.data.type === 'shf-play-chime' &&
                    window.SHFPush && typeof window.SHFPush.playChime === 'function') {
                    window.SHFPush.playChime();
                }
            });
    ...
    <script>
        (function() {
            ...
            if (fabMain && !fabMain.dataset.shfBound) {
                ...
                var fabMenu = document.querySelector('.shf-fab-menu');
                // The .shf-fab-wrap has pointer-events:none and the backdrop sits
                // at z-index 190. While the menu is open, lift it above the
                // backdrop (z-index needs a positioning context) and re-enable
                // clicks, so taps land on the items instead of the backdrop.
                var openMenu = function() {
                    if (!fabMenu) return;
                    fabMenu.style.position = 'relative';
                    fabMenu.style.zIndex = '210';
                    fabMenu.style.pointerEvents = 'auto';
                };
                var resetMenu = function() {
                    if (!fabMenu) return;
                    fabMenu.style.position = '';
                    fabMenu.style.zIndex = '';
