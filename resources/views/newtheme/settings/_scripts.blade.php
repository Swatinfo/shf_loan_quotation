        <script>
            $(function() {
                ...
                function activateTab(tab) {
                    ...
                    $('#tab-' + tab).show();
                    try {
                        localStorage.setItem(SETTINGS_TAB_STORAGE, tab);
                    } catch (e) {
                        /* storage disabled */
                    }
                ...
                $(document).on('click', '.shf-tab, .tabs .tab[data-tab], .settings-nt-rail-item[data-tab]', function(
                    e) {
                    e.preventDefault();
                ...
                if (hash) {
                    restored = activateTab(hash);
                }
                if (!restored) {
                    try {
                        var stored = localStorage.getItem(SETTINGS_TAB_STORAGE);
                        if (stored) {
                            restored = activateTab(stored);
                        }
                    } catch (e) {
                        /* storage disabled */
                    }
                ...
                $('#tab-charges form').on('submit', function(e) {
                    if (!SHF.validateForm($(this), {
                            'iomCharges[thresholdAmount]': {
                                required: true,
                                numeric: true,
                                min: 0,
                                label: 'Threshold Amount'
                            },
                            'iomCharges[fixedCharge]': {
                                required: true,
                                numeric: true,
                                min: 0,
                                label: 'Fixed Charge'
                            },
                            'iomCharges[percentageAbove]': {
                                required: true,
                                numeric: true,
                                min: 0,
                                label: 'Percentage'
                ...
                function renderDocList(type) {
                    ...
                    $.each(docs[type].en, function(idx, enVal) {
                        ...
                        html +=
                            ...
                            '<input type="text" name="documents_en[' + type + '][]" value="' + esc(enVal) +
                            '" class="shf-input w-100 small" placeholder="English">' +
                            '</div>' +
                            '<div class="col-12 col-md">' +
                            '<input type="text" name="documents_gu[' + type + '][]" value="' + esc(guVal) +
                            '" class="shf-input w-100 small" placeholder="Gujarati">' +
                            '</div>' +
                            '<div class="col-12 col-md-auto">' +
                            '<button type="button" class="btn-accent-sm removeDocBtn shf-btn-danger w-100 w-md-auto justify-content-center" data-type="' +
                            type + '" data-idx="' + idx + '">' +
                ...
                function switchDocTab(type) {
                    ...
                    $('#docPane-' + type).show();
                    renderDocList(type);
                    $('.doc-sub-tab[data-doc-type="' + type + '"]').addClass('active');
                }
                ...
                $(document).on('click', '.doc-sub-tab', function() {
                    $('.doc-sub-tab').removeClass('active');
                ...
                function renderDvrList(items, containerId, inputPrefix) {
                    ...
                    $.each(items, function(idx, item) {
                        html +=
                            '<div class="shf-reason-row row g-2 align-items-center p-2 rounded" style="background:var(--bg);border:1px solid var(--border);margin:0 0 4px 0;">' +
                            '<input type="hidden" name="' + inputPrefix + '[' + idx + '][key]" value="' + esc(
                                item.key) + '">' +
                            '<div class="col-auto d-flex align-items-center gap-2" style="min-width:0;">' +
                            '<span class="fw-bold shf-text-xs shf-text-gray-light">' + (idx + 1) + '.</span>' +
                            '<span class="shf-badge shf-badge-gray shf-text-2xs text-truncate" style="max-width:10rem;" title="' +
                            esc(item.key) + '">' + esc(item.key) + '</span>' +
                            '</div>' +
                            '<div class="col-12 col-md">' +
                            '<input type="text" name="' + inputPrefix + '[' + idx + '][label_en]" value="' +
                            esc(item.label_en) + '" class="shf-input w-100 small" placeholder="English">' +
                            '</div>' +
                            '<div class="col-12 col-md">' +
                            '<input type="text" name="' + inputPrefix + '[' + idx + '][label_gu]" value="' +
                            esc(item.label_gu) + '" class="shf-input w-100 small" placeholder="Gujarati">' +
                            '</div>' +
                            '<div class="col-12 col-md-auto">' +
                            '<button type="button" class="btn-accent-sm shf-btn-danger dvr-remove-btn w-100 w-md-auto justify-content-center" data-list="' +
                            containerId + '" data-idx="' + idx + '">' +
                ...
                $(document).on('click', '.dvr-remove-btn', function() {
                    ...
                    if (listId === 'dvrContactTypesList') {
                        ...
                        renderDvrList(dvrPurposes, 'dvrPurposesList', 'dvrPurposes');
                    } else if (listId === 'quotationReferralTypesList') {
                        quotationReferralTypes.splice(idx, 1);
                        renderDvrList(quotationReferralTypes, 'quotationReferralTypesList',
                ...
                function renderReasonList(items, containerId, inputPrefix, datalistId) {
                    ...
                    groupOrder.forEach(function(groupName) {
                        html += '<div class="shf-form-label mt-2 mb-1" style="color:var(--accent);">' +
                        ...
                        groups[groupName].forEach(function(entry) {
                            ...
                            html +=
                                '<div class="shf-reason-row row g-2 align-items-center p-2 rounded" style="background:var(--bg);border:1px solid var(--border);margin:0 0 4px 0;">' +
                                '<input type="hidden" name="' + inputPrefix + '[' + idx +
                                '][key]" value="' + escapeHtml(item.key) + '">' +
                                '<div class="col-auto d-flex align-items-center gap-2" style="min-width:0;">' +
                                '<span class="fw-bold shf-text-xs shf-text-gray-light">' + (idx + 1) +
                                '.</span>' +
                                '<span class="shf-badge shf-badge-gray shf-text-2xs text-truncate" style="max-width:10rem;" title="' +
                                escapeHtml(item.key) + '">' + escapeHtml(item.key) + '</span>' +
                                '</div>' +
                                '<div class="col-12 col-md-6 col-lg">' +
                                '<input type="text" name="' + inputPrefix + '[' + idx +
                                '][label_en]" value="' + escapeHtml(item.label_en) +
                                '" class="shf-input w-100 small" placeholder="English">' +
                                '</div>' +
                                '<div class="col-12 col-md-6 col-lg">' +
                                '<input type="text" name="' + inputPrefix + '[' + idx +
                                '][label_gu]" value="' + escapeHtml(item.label_gu) +
                                '" class="shf-input w-100 small" placeholder="Gujarati">' +
                                '</div>' +
                                '<div class="col-12 col-md-8 col-lg-3">' +
                                '<input type="text" name="' + inputPrefix + '[' + idx +
                                '][group]" value="' + escapeHtml(item.group || 'Other') +
                                '" class="shf-input w-100 small" placeholder="Group" list="' +
                                datalistId + '">' +
                                '</div>' +
                                '<div class="col-12 col-md-4 col-lg-auto text-md-end">' +
                                '<button type="button" class="btn-accent-sm shf-btn-danger reason-remove-btn w-100 w-md-auto justify-content-center" data-list="' +
                                containerId + '" data-idx="' + idx + '">' +
                                '<svg class="shf-icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>' +
                ...
                renderReasonList(quotationHoldReasons, 'quotationHoldReasonsList', 'quotationHoldReasons',
                    'holdGroupOptions');
                renderReasonList(quotationCancelReasons, 'quotationCancelReasonsList', 'quotationCancelReasons',
                    'cancelGroupOptions');
                ...
                $('#addHoldReasonBtn').on('click', function() {
                    if (pushReason(quotationHoldReasons, $('#newHoldReasonKey'), $('#newHoldReasonEn'), $(
                            '#newHoldReasonGu'), $('#newHoldReasonGroup'))) {
                        renderReasonList(quotationHoldReasons, 'quotationHoldReasonsList',
                            'quotationHoldReasons', 'holdGroupOptions');
                    }
                });

                $('#addCancelReasonBtn').on('click', function() {
                    if (pushReason(quotationCancelReasons, $('#newCancelReasonKey'), $('#newCancelReasonEn'), $(
                            '#newCancelReasonGu'), $('#newCancelReasonGroup'))) {
                        renderReasonList(quotationCancelReasons, 'quotationCancelReasonsList',
                            'quotationCancelReasons', 'cancelGroupOptions');
                    }
                });
                ...
                // ============================================================
                //  QUOTATION REFERRAL TYPES MANAGER (same shape as DVR types)
                // ============================================================
                var quotationReferralTypes = @json($config['quotationReferralTypes'] ?? []);
                renderDvrList(quotationReferralTypes, 'quotationReferralTypesList', 'quotationReferralTypes');

                $('#addReferralTypeBtn').on('click', function() {
                    var key = $.trim($('#newReferralTypeKey').val());
                    var en = $.trim($('#newReferralTypeEn').val());
                    var gu = $.trim($('#newReferralTypeGu').val());
                    if (key && en) {
                        key = key.toLowerCase().replace(/[^a-z0-9_]/g, '_');
                        quotationReferralTypes.push({
                            key: key,
                            label_en: en,
                            label_gu: gu || en
                        });
                        renderDvrList(quotationReferralTypes, 'quotationReferralTypesList',
                            'quotationReferralTypes');
                        $('#newReferralTypeKey, #newReferralTypeEn, #newReferralTypeGu').val('');
                    }
                });

                // Auto-add pending row on form submit
                $('#quotationReferralTypesForm').on('submit', function() {
                    var key = $.trim($('#newReferralTypeKey').val());
                    var en = $.trim($('#newReferralTypeEn').val());
                    if (key && en) {
                        var gu = $.trim($('#newReferralTypeGu').val());
