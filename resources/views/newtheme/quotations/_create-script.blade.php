        <script>
            ...
            function onCustomerTypeChange() {
                ...
                // The section starts with .shf-collapse-hidden, which has
                // `display: none !important` in quotation-create.css — inline
                // style.display is overridden by !important, so we must
                // toggle the class itself.
                if (!type) {
                    docSection.classList.add('shf-collapse-hidden');
                    return;
                }
                docSection.classList.remove('shf-collapse-hidden');
                ...
                docGrid.innerHTML = enDocs.map((doc, i) => {
                    ...
                    return renderDocRow(doc, biName, false, i);
                }).join('');
            }

            // Render a single doc row. Unchecked rows render with strike-through
            // styling (.shf-doc-struck) so the operator can see what's excluded
            // — they aren't hidden, just visually struck. The excluded flag is
            // posted with every row so the server persists both included and
            // excluded entries.
            function renderDocRow(en, biName, excluded, idx) {
                const cls = excluded ? 'shf-doc-row shf-doc-struck' : 'shf-doc-row';
                const checked = excluded ? '' : 'checked';
                return `<div class="col-md-6"><label class="${cls} d-flex align-items-center gap-2 p-2 rounded shf-clickable" id="docItem${idx}">
                    <input type="checkbox" ${checked} id="docCheck${idx}" data-en="${en.replace(/"/g, '&quot;')}" style="accent-color: #f15a29;" onchange="toggleDocStruck(this)">
                    <span class="small">${biName}</span>
                </label></div>`;
            }

            // Click handler — flip the parent label's struck class so the user
            // sees a line-through preview before submitting.
            function toggleDocStruck(cb) {
                const label = cb.closest('.shf-doc-row');
                if (!label) return;
                if (cb.checked) {
                    label.classList.remove('shf-doc-struck');
                } else {
                    label.classList.add('shf-doc-struck');
                }
            }

            // Returns every doc row with its excluded flag — replaces the old
            // getSelectedDocuments() which silently dropped unchecked rows.
            function getDocumentRows() {
                const docs = [];
                document.querySelectorAll('#docGrid input[type="checkbox"]').forEach((cb, i) => {
            ...
            function renderTenureChips() {
                ...
                if (!selectedBanks.length) {
                    selDiv.classList.add('shf-collapse-hidden');
                    return;
                }
                selDiv.classList.remove('shf-collapse-hidden');
            ...
            async function generatePDF() {
                ...
                const payload = {
                    ...
                    banks: bankData,
                    selectedTenures,
                    documents: getDocumentRows(),
                    additionalNotes: document.getElementById('additionalNotes').value.trim(),
                    ourServices: CONFIG.ourServices || '',
                    // Admin / super_admin / bdh may override the creator user;
                    // the controller ignores this field for anyone else.
                    createdByUserId: document.getElementById('quotationCreatedBy')?.value || null,
                };

                // Edit mode: PUT to the update URL injected by edit.blade.php.
                // Create mode: POST to the generate route. Update doesn't
                // download a fresh PDF inline — it redirects to the show page
                // where the user can hit "Branded PDF" / "Plain PDF".
                const editUrl = window.SHF_QUOTATION_UPDATE_URL || null;
                const submitUrl = editUrl || '{{ route('quotations.generate') }}';
                const submitMethod = editUrl ? 'PUT' : 'POST';

                try {
                    // fetch() bypasses the global jQuery loader hooks — drive
                    // SHF.loader manually so the (slow) PDF build shows progress.
                    let resp, result;
            ...
            (async function() {
                ...
                // Edit mode: apply prefill data after the standard init sequence.
                // window.SHF_QUOTATION_PREFILL is injected by edit.blade.php and
                // contains every field needed to reconstruct an existing
                // quotation: customer, loan, banks/charges/ROI, tenures, docs
                // (with their excluded flags), notes, prepared-by, branch.
                if (window.SHF_QUOTATION_PREFILL) {
                    applyQuotationPrefill(window.SHF_QUOTATION_PREFILL);
                }
            })();

            function applyQuotationPrefill(data) {
                // Simple inputs first.
                ...
                if (data.loan_amount) {
                    ...
                    amt.value = formatIndianNumber(parseInt(data.loan_amount));
                    document.getElementById('loanWords').value = getBilingualAmountWords(parseInt(data.loan_amount));
                }
                if (data.additional_notes) document.getElementById('additionalNotes').value = data.additional_notes;
                if (data.referral_name) document.getElementById('referralName').value = data.referral_name;
                if (data.referral_type) document.getElementById('referralType').value = data.referral_type;
                if (data.prepared_by_name) document.getElementById('preparedByName').value = data.prepared_by_name;
                if (data.prepared_by_mobile) document.getElementById('preparedByMobile').value = data.prepared_by_mobile;
                if (data.location_id) document.getElementById('quotationLocation').value = data.location_id;
                if (data.branch_id) document.getElementById('quotationBranch').value = data.branch_id;

                // Customer type triggers the doc grid to render — we then
                // overwrite it with the persisted rows (which carry their
                // excluded flag) so the user sees the same struck rows that
                // were saved.
                if (data.customer_type) {
                    ...
                    if (Array.isArray(data.documents) && data.documents.length) {
                        const docGrid = document.getElementById('docGrid');
                        docGrid.innerHTML = data.documents.map((d, i) => {
                            const biName = d.gu ? `${d.en} / ${d.gu}` : d.en;
                            return renderDocRow(d.en, biName, !!d.excluded, i);
                        }).join('');
                    }
                }

                // Selected tenures — applied after bank toggle since tenure
                // chips only render when at least one bank is selected.
                const desiredTenures = Array.isArray(data.selected_tenures) ? data.selected_tenures.map(Number) : [];

                // Banks: trigger toggleBank for each saved bank, then populate
                // their ROI + charges fields. selectedBanks order matters
                // because the bank cards key off index — toggleBank() pushes
                // in click order, so we iterate the saved order.
                const banks = Array.isArray(data.banks) ? data.banks : [];
                banks.forEach(b => {
                    if (!selectedBanks.includes(b.bank_name)) {
                        toggleBank(b.bank_name);
                    }
                });

                // After the bank cards render, populate ROI + charges per bank.
                // toggleBank already triggered renderBankCards.
                banks.forEach(b => {
                    const idx = selectedBanks.indexOf(b.bank_name);
                    if (idx < 0) return;
                    const p = 'bank_' + idx;
                    const set = (id, val) => {
                        const el = document.getElementById(id);
                    ...
                    set(p + '_roiMin', b.roi_min);
                    set(p + '_roiMax', b.roi_max);
                    set(p + '_pf', b.pf_charge);
                    set(p + '_admin', b.admin_charge ? formatIndianNumber(parseInt(b.admin_charge)) : '');
                    set(p + '_stamp_notary', b.stamp_notary ? formatIndianNumber(parseInt(b.stamp_notary)) : '');
                    set(p + '_registration_fee', b.registration_fee ? formatIndianNumber(parseInt(b.registration_fee)) :
                        '');
                    set(p + '_advocate', b.advocate_fees ? formatIndianNumber(parseInt(b.advocate_fees)) : '');
                    set(p + '_iom', b.iom_charge ? formatIndianNumber(parseInt(b.iom_charge)) : '');
                    set(p + '_tc', b.tc_report ? formatIndianNumber(parseInt(b.tc_report)) : '');
                    set(p + '_extraName1', b.extra1_name || '');
                    set(p + '_extraAmt1', b.extra1_amount ? formatIndianNumber(parseInt(b.extra1_amount)) : '');
                    set(p + '_extraName2', b.extra2_name || '');
                    set(p + '_extraAmt2', b.extra2_amount ? formatIndianNumber(parseInt(b.extra2_amount)) : '');
                    if (typeof updateEMI === 'function') updateEMI(idx);
                    if (typeof updatePfAmount === 'function') updatePfAmount(idx);
                    if (typeof updateAdminGst === 'function') updateAdminGst(idx);
                });

                // Sync tenure checkboxes to the saved set.
                if (desiredTenures.length) {
                    document.querySelectorAll('#tenureChips input[type="checkbox"]').forEach(cb => {
                        cb.checked = desiredTenures.includes(parseInt(cb.value));
                    });
                    if (typeof updateTenureVisuals === 'function') updateTenureVisuals();
                }
            }
        </script>
