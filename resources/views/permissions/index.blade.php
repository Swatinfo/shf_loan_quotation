            ...
            <div class="shf-section">
                ...
                <div class="shf-section-body">
                    ...
                    <p class="small mb-4 shf-text-gray">
                        <svg style="width:14px;height:14px;display:inline;vertical-align:-2px;margin-right:4px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Loan permissions are managed separately in <a href="{{ route('loan-settings.index') }}#role-permissions" style="color: var(--accent); font-weight: 600;">Loan Settings → Role Permissions</a>
                    </p>
                    ...
                    <form method="POST" action="{{ route('permissions.update') }}">
                                ...
                                <tbody>
                                    @foreach($permissions as $group => $perms)
                                        <tr class="bg-accent-dim">
                                            <td colspan="{{ count($roles) + 1 }}" class="font-display fw-semibold small shf-settings-tab-header">
                                                {{ $group }}
                                            </td>
                                        </tr>
                                        @foreach($perms as $perm)
                                            <tr>
                                                <td>
                                                    ...
                                                    @if($perm->description)
                                                        <span class="d-block small shf-text-gray-light">{{ $perm->description }}</span>
                                                    @endif
                                                </td>
                                                @foreach($roles as $role)
                        ...
                        <div class="d-flex justify-content-end gap-3 mt-4">
                            <a href="{{ route('settings.index') }}" class="btn-accent-outline"><svg class="shf-icon-md" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg> Cancel</a>
